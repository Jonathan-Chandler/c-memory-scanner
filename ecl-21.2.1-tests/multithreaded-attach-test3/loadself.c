...
int main_test(void) 
{
  ...
  hThreadLaunchPreload = CreateRemoteThread(hProcess, NULL, 0, (LPTHREAD_START_ROUTINE)lpLoadLibAddr, lpAllocatedDllPath, 0, NULL);
  ...
  hThreadLaunchPreload = CreateRemoteThread(hProcess, NULL, 0, (LPTHREAD_START_ROUTINE)lpLoadAppMain, NULL, 0, NULL);
  if(hThreadLaunchPreload == NULL) 
  {
  ...
  // start thread
  hThreadLaunchPreload = CreateRemoteThread(hProcess, NULL, 0, (LPTHREAD_START_ROUTINE)lpLoadLibAddr, lpAllocatedDllPath2, 0, NULL);
  if(hThreadLaunchPreload == NULL) 
  {
    printf("Error: the remote thread could not be created at app_reconfigure_path for preload.dll\n");
    retval = -ENXIO;
  ...
  // start thread
  hThreadLaunchPreload = CreateRemoteThread(hProcess, NULL, 0, (LPTHREAD_START_ROUTINE)lpLaunchSwank, NULL, 0, NULL);
  if(hThreadLaunchPreload == NULL) 
  {
