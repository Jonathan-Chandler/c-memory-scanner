...
#include "C:/work/c/memscan-dll/ecl-21.2.1-tests/ecl-rebuild/build/lsp/helpfile.eclh"
...
static cl_object L1new_documentation_pool(cl_narg narg, ...)
{
 ...
 if (ecl_unlikely(narg>1)) FEwrong_num_arguments_anonym();
 {
  ...
  T1 = cl_make_hash_table(4, ECL_SYM("TEST",1343), T0, ECL_SYM("SIZE",1335), v1size);
  cl_set(ECL_SYM("*DOCUMENTATION-POOL*",1835),CONS(T1,ecl_symbol_value(ECL_SYM("*DOCUMENTATION-POOL*",1835))));
  value0 = ecl_symbol_value(ECL_SYM("*DOCUMENTATION-POOL*",1835));
...
TTL:
  {
   cl_object v5dict;
   v5dict = ecl_car(ecl_symbol_value(ECL_SYM("*DOCUMENTATION-POOL*",1835)));
...
TTL:
  {
   cl_object v4dict;
   v4dict = ecl_car(ecl_symbol_value(ECL_SYM("*DOCUMENTATION-POOL*",1835)));
...
static cl_object L8get_annotation(cl_narg narg, cl_object v1object, cl_object v2key, ...)
{
 ...
 if (ecl_unlikely(narg>3)) FEwrong_num_arguments_anonym();
 {
  ...
  va_end(args);
  {
   ...
   {
    cl_object v5;
    v5 = ecl_symbol_value(ECL_SYM("*DOCUMENTATION-POOL*",1835));
    goto L6;
L5:;
    {
     cl_object v6dict;
...
L12:;
      ...
      v7record = ecl_function_dispatch(cl_env_copy,VV[31])(2, v1object, v6dict) /*  SEARCH-HELP-FILE */;
      goto L11;
L14:;
      v7record = ECL_NIL;
L11:;
...
static cl_object L9dump_documentation(cl_narg narg, cl_object v1file, ...)
{
 ...
 if (ecl_unlikely(narg>2)) FEwrong_num_arguments_anonym();
 {
  ...
  {
   cl_object v3dict;
   v3dict = ecl_car(ecl_symbol_value(ECL_SYM("*DOCUMENTATION-POOL*",1835)));
   if (Null(cl_hash_table_p(v3dict))) { goto L3; }
   ecl_function_dispatch(cl_env_copy,VV[33])(3, v3dict, v1file, v2merge) /*  DUMP-HELP-FILE */;
   {
    cl_object v4;
    v4 = ecl_symbol_value(ECL_SYM("*DOCUMENTATION-POOL*",1835));
...
/*      optimize speed 1, debug 1, space 1, safety 2                  */
static cl_object L10get_documentation(cl_object v1object, cl_object v2doc_type)
{
 cl_object T0;
 cl_object env0 = ECL_NIL;
 const cl_env_ptr cl_env_copy = ecl_process_env();
...
L1:;
  ...
  return value0;
L5:;
  value0 = L8get_annotation(3, v1object, ECL_SYM("DOCUMENTATION",314), v2doc_type);
  return value0;
 }
}
/*      function definition for SET-DOCUMENTATION                     */
/*      optimize speed 1, debug 1, space 1, safety 2                  */
static cl_object L11set_documentation(cl_object v1object, cl_object v2doc_type, cl_object v3string)
{
 cl_object env0 = ECL_NIL;
 const cl_env_ptr cl_env_copy = ecl_process_env();
 cl_object value0;
...
TTL:
  ...
  cl_error(2, VV[13], v3string);
L1:;
  {
   cl_object v4key;
   v4key = ECL_SYM("DOCUMENTATION",314);
...
/*      optimize speed 1, debug 1, space 1, safety 2                  */
static cl_object L12expand_set_documentation(cl_object v1symbol, cl_object v2doc_type, cl_object v3string)
{
 cl_object T0, T1, T2;
 cl_object env0 = ECL_NIL;
 const cl_env_ptr cl_env_copy = ecl_process_env();
...
TTL:
  ...
  cl_error(2, VV[13], v3string);
L4:;
  T0 = cl_list(2, ECL_SYM("QUOTE",681), v1symbol);
  T1 = cl_list(2, ECL_SYM("QUOTE",681), v2doc_type);
  T2 = cl_list(4, VV[14], T0, T1, v3string);
  value0 = ecl_list1(T2);
  cl_env_copy->nvalues = 1;
  return value0;
L1:;
...
/*      optimize speed 1, debug 1, space 1, safety 2                  */
static cl_object L13make_dspec(cl_object v1definition)
{
 cl_object T0;
 cl_object env0 = ECL_NIL;
 const cl_env_ptr cl_env_copy = ecl_process_env();
...
TTL:
  if (!(ECL_CONSP(v1definition))) { goto L1; }
  {
   ...
   v4extra = ECL_NIL;
   if (!((v2kind)==(ECL_SYM("DEFMETHOD",942)))) { goto L6; }
   {
    cl_object v5list;
...
/*      optimize speed 1, debug 1, space 1, safety 2                  */
static cl_object LC14optional_annotation(cl_object v1whole, cl_object v2env)
{
 cl_object T0;
 cl_object env0 = ECL_NIL;
 const cl_env_ptr cl_env_copy = ecl_process_env();
...
/*      optimize speed 1, debug 1, space 1, safety 2                  */
static cl_object L15default_annotation_logic(cl_narg narg, cl_object v1source_location, cl_object v2definition, cl_object v3output_form, ...)
{
 ...
 if (ecl_unlikely(narg>4)) FEwrong_num_arguments_anonym();
 {
  ...
  va_list args; va_start(args,v3output_form);
  {
   int i = 3;
   if (i >= narg) {
    v4dspec = L13make_dspec(v2definition);
   } else {
    i++;
  ...
  va_end(args);
  {
   ...
   T0 = cl_list(2, ECL_SYM("QUOTE",681), v6name);
   T1 = cl_list(2, ECL_SYM("QUOTE",681), v4dspec);
   T2 = cl_list(2, ECL_SYM("QUOTE",681), v1source_location);
   T3 = cl_list(5, VV[17], T0, VV[18], T1, T2);
   if (!((v5kind)==(ECL_SYM("DEFUN",291)))) { goto L10; }
   goto L7;
   goto L8;
L10:;
   goto L8;
L8:;
   if (!((v5kind)==(ECL_SYM("DEFMACRO",285)))) { goto L14; }
   goto L7;
   goto L12;
L14:;
   goto L12;
L12:;
   if (!((v5kind)==(ECL_SYM("DEFGENERIC",940)))) { goto L5; }
   goto L6;
L7:;
L6:;
   T5 = cl_list(2, ECL_SYM("QUOTE",681), v6name);
   T6 = ecl_caddr(v2definition);
   T7 = cl_list(2, ECL_SYM("QUOTE",681), T6);
   T4 = cl_list(5, VV[17], T5, ECL_SYM("LAMBDA-LIST",1000), ECL_NIL, T7);
   goto L4;
L5:;
   T4 = ECL_NIL;
L4:;
   value0 = cl_list(4, ECL_SYM("PROGN",673), T3, T4, v3output_form);
   return value0;
  }
 }
}

#include "C:/work/c/memscan-dll/ecl-21.2.1-tests/ecl-rebuild/build/lsp/helpfile.data"
#ifdef __cplusplus
extern "C"
#endif
ECL_DLLEXPORT void _eclleskaGb7_Y8VZTr51(cl_object flag)
{
 const cl_env_ptr cl_env_copy = ecl_process_env();
 cl_object value0;
 cl_object *VVtemp;
 ...
 flag->cblock.source = ecl_make_constant_base_string("SRC:LSP;HELPFILE.LSP.NEWEST",-1);
 return;}
 #ifdef ECL_DYNAMIC_VV
 VV = Cblock->cblock.data;
 #endif
 Cblock->cblock.data_text = (const cl_object *)"@EcLtAg:_eclleskaGb7_Y8VZTr51@";
  ...
  si_select_package(VVtemp[0]);
 {
  ...
  T1 = cl_make_hash_table(4, ECL_SYM("TEST",1343), T0, ECL_SYM("SIZE",1335), ecl_make_fixnum(128));
  cl_set(ECL_SYM("*DOCUMENTATION-POOL*",1835),cl_list(2, T1, VVtemp[1]));
  si_Xmake_special(VV[0]);
  cl_set(VV[0],ECL_T);
 }
  ecl_cmp_defun(VV[23]);                          /*  NEW-DOCUMENTATION-POOL */
  ecl_cmp_defun(VV[24]);                          /*  RECORD-CONS     */
  ecl_cmp_defun(VV[25]);                          /*  RECORD-FIELD    */
  ecl_cmp_defun(VV[26]);                          /*  SET-RECORD-FIELD */
  ecl_cmp_defun(VV[27]);                          /*  REM-RECORD-FIELD */
  ecl_cmp_defun(VV[28]);                          /*  ANNOTATE        */
  ecl_cmp_defun(VV[29]);                          /*  REMOVE-ANNOTATION */
  ecl_cmp_defun(VV[30]);                          /*  GET-ANNOTATION  */
  ecl_cmp_defun(VV[32]);                          /*  DUMP-DOCUMENTATION */
  ecl_cmp_defun(VV[34]);                          /*  GET-DOCUMENTATION */
  ecl_cmp_defun(VV[35]);                          /*  SET-DOCUMENTATION */
  ecl_cmp_defun(VV[36]);                          /*  EXPAND-SET-DOCUMENTATION */
  ecl_cmp_defun(VV[37]);                          /*  MAKE-DSPEC      */
  ecl_cmp_defmacro(VV[38]);                       /*  OPTIONAL-ANNOTATION */
  ecl_cmp_defun(VV[39]);                          /*  DEFAULT-ANNOTATION-LOGIC */
  cl_set(ECL_SYM("*REGISTER-WITH-PDE-HOOK*",1203),VV[22]);
}
