#line 1 "C:/work/c/memscan-dll/ecl-21.2.1-tests/ecl-rebuild/src/c/cfun.d"
...
{
  si_fset(2, sym,
          ecl_make_cfun(c_function, sym, ecl_symbol_value(ECL_SYM("SI::*CBLOCK*",1015)), narg));
}
...
{
  si_fset(3, sym,
          ecl_make_cfun(c_function, sym, ecl_symbol_value(ECL_SYM("SI::*CBLOCK*",1015)), 2),
          ECL_T);
}
...
{
  si_fset(3, sym,
          ecl_make_cfun_va(c_function, sym, ecl_symbol_value(ECL_SYM("SI::*CBLOCK*",1015)), narg_fixed),
          ECL_T);
}
...
cl_function_lambda_expression(cl_object fun)
{
  ...
  case t_bytecodes:
    ...
    if (name == ECL_NIL && output != ECL_NIL)
      output = cl_cons(ECL_SYM("LAMBDA",454), output);
    else if (name != ECL_SYM("SI::BYTECODES",1714) && output != ECL_NIL)
      output = cl_listX(3, ECL_SYM("EXT::LAMBDA-BLOCK",1367), name, output);
    break;
  case t_cfun:
  case t_cfunfixed:
    name = fun->cfun.name;
