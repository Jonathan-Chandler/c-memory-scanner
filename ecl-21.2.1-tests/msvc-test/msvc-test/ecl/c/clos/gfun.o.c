#line 1 "C:/work/c/memscan-dll/ecl-21.2.1-tests/ecl-rebuild/src/c/clos/gfun.d"
...
clos_set_funcallable_instance_function(cl_object x, cl_object function_or_t)
{
  if (ecl_unlikely(!ECL_INSTANCEP(x)))
    FEwrong_type_nth_arg(ecl_make_fixnum(/*CLOS::SET-FUNCALLABLE-INSTANCE-FUNCTION*/1601),
                         1, x, ecl_make_fixnum(/*EXT::INSTANCE*/1370));
...
#line 110
	#line 110
	cl_object __value0 = ((ECL_INSTANCEP(x) && (x->instance.isgf))? ECL_T : ECL_NIL);
#line 110
	the_env->nvalues = 1;
#line 110
	the_env->values[0] = __value0;
...
generic_compute_applicable_method(cl_env_ptr env, cl_object frame, cl_object gf)
{
  /* method not cached */
  cl_object memoize;
  cl_object methods = _ecl_funcall3(ECL_SYM("CLOS::COMPUTE-APPLICABLE-METHODS-USING-CLASSES",1556),
                                    gf, frame_to_classes(frame));
  unlikely_if (Null(memoize = env->values[1])) {
    cl_object arglist = frame_to_list(frame);
    methods = _ecl_funcall3(ECL_SYM("COMPUTE-APPLICABLE-METHODS",938),
                            gf, arglist);
...
_ecl_standard_dispatch(cl_object frame, cl_object gf)
{
  ...
  if (func == ECL_NIL)
    func = cl_apply(3, ECL_SYM("NO-APPLICABLE-METHOD",961), gf, frame);
  else
