#line 1 "C:/work/c/memscan-dll/ecl-21.2.1-tests/ecl-rebuild/src/c/clos/accessor.d"
...
{
  env->values[0] = cl_apply(3, ECL_SYM("NO-APPLICABLE-METHOD",961), gfun, args);
}
...
{
  cl_object methods = _ecl_funcall3(ECL_SYM("COMPUTE-APPLICABLE-METHODS",938),
                                    gfun, args);
  unlikely_if (Null(methods)) {
    return OBJNULL;
  } else {
    cl_object first = ECL_CONS_CAR(methods);
    cl_object slotd = _ecl_funcall3(ECL_SYM("SLOT-VALUE",973), first,
                                    ECL_SYM("CLOS::SLOT-DEFINITION",1603));
    return _ecl_funcall3(ECL_SYM("SLOT-VALUE",973), slotd, ECL_SYM("CLOS::NAME",1596));
  }
}
...
slot_method_index(cl_object gfun, cl_object instance, cl_object args)
{
  ...
  unlikely_if (slot_name == OBJNULL)
    ...
    cl_object table = _ecl_funcall3(ECL_SYM("SLOT-VALUE",973),
                                    ECL_CLASS_OF(instance),
                                    ECL_SYM("CLOS::LOCATION-TABLE",1588));
...
ensure_up_to_date_instance(cl_object instance)
{
  if (si_instance_obsolete_p(instance) == ECL_T) {
    _ecl_funcall2(ECL_SYM("CLOS::UPDATE-INSTANCE",1635), instance);
  }
}
...
ecl_slot_reader_dispatch(cl_narg narg, ... /* cl_object instance */)
{
  ...
  unlikely_if (value == ECL_UNBOUND) {
    ...
    value = _ecl_funcall4(ECL_SYM("SLOT-UNBOUND",972),
                          ECL_CLASS_OF(instance),
                          instance,
                          slot_name);
  }
