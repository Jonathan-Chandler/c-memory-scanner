#line 1 "C:/work/c/memscan-dll/ecl-21.2.1-tests/ecl-rebuild/src/c/threads/queue.d"
...
ecl_wait_on_timed(cl_env_ptr env, mp_wait_test condition, cl_object mp_object)
{
  ...
  ECL_UNWIND_PROTECT_BEGIN(the_env) {
    ...
    ecl_bds_bind(the_env, ECL_SYM("EXT::*INTERRUPTS-ENABLED*",7), ECL_T);
      ...
      if (p->process.phase == ECL_PROCESS_INACTIVE ||
        ...
        *tail = ECL_CONS_CDR(l);
        ...
        if (flags & ECL_WAKEUP_KILL)
          ecl_interrupt_process(p, ECL_SYM("MP::EXIT-PROCESS",1434));
        else
