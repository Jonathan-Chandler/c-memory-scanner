#line 1 "C:/work/c/memscan-dll/ecl-21.2.1-tests/ecl-rebuild/src/c/number.d"
...
ecl_to_bit(cl_object x) {
  if (ecl_unlikely((x != ecl_make_fixnum(0)) && (x != ecl_make_fixnum(1))))
    FEwrong_type_nth_arg(ecl_make_fixnum(/*COERCE*/233), 1, x, ecl_make_fixnum(/*BIT*/125));
...
ecl_to_uint8_t(cl_object x) {
  ...
  FEwrong_type_argument(cl_list(2, ECL_SYM("UNSIGNED-BYTE",887), ecl_make_fixnum(8)),
                        x);
}
...
ecl_to_int8_t(cl_object x) {
  ...
  FEwrong_type_argument(cl_list(2, ECL_SYM("SIGNED-BYTE",763), ecl_make_fixnum(8)),
                        x);
}
...
ecl_to_ushort(cl_object x) {
  ...
  FEwrong_type_argument(cl_list(3,ECL_SYM("INTEGER",439),
                                ecl_make_fixnum(0),
...
ecl_make_complex(cl_object r, cl_object i)
{
  ...
  if (!ECL_REAL_TYPE_P(tr)) { ecl_type_error(ECL_SYM("COMPLEX",243), "real part", r, ECL_SYM("REAL",705)); }
  if (!ECL_REAL_TYPE_P(ti)) { ecl_type_error(ECL_SYM("COMPLEX",243), "imaginary part", i, ECL_SYM("REAL",705)); }
  switch((tr > ti) ? tr : ti) {
...
si_complex_float(cl_object r, cl_object i)
{
  ...
  switch (tr) {
  case t_singlefloat:
    if (ti != tr) { ecl_type_error(ECL_SYM("SI::COMPLEX-FLOAT",1871),"imag part", i, ECL_SYM("SINGLE-FLOAT",780)); }
    result = ecl_alloc_object(t_csfloat);
    ecl_csfloat(result) = CMPLXF(ecl_single_float(r), ecl_single_float(i));
    break;
  case t_doublefloat:
    if (ti != tr) { ecl_type_error(ECL_SYM("SI::COMPLEX-FLOAT",1871),"imag part", i, ECL_SYM("DOUBLE-FLOAT",317)); }
    result = ecl_alloc_object(t_cdfloat);
    ecl_cdfloat(result) = CMPLX(ecl_double_float(r), ecl_double_float(i));
    break;
  case t_longfloat:
    if (ti != tr) { ecl_type_error(ECL_SYM("SI::COMPLEX-FLOAT",1871),"imag part", i, ECL_SYM("LONG-FLOAT",510)); }
    result = ecl_alloc_object(t_clfloat);
    ecl_clfloat(result) = CMPLXL(ecl_long_float(r), ecl_long_float(i));
    break;
  default:
    ecl_type_error(ECL_SYM("SI::COMPLEX-FLOAT",1871),"real part", r, ECL_SYM("FLOAT",376));
  }
  return result;
}
...
ecl_to_float(cl_object x)
{
  ...
  default:
    FEwrong_type_nth_arg(ecl_make_fixnum(/*COERCE*/233), 1, x, ecl_make_fixnum(/*REAL*/705));
  }
}
...
float _Complex ecl_to_csfloat(cl_object x) {
  ...
  default:
    FEwrong_type_nth_arg(ecl_make_fixnum(/*COERCE*/233), 1, x, ecl_make_fixnum(/*NUMBER*/608));
  }
}
...
cl_rational(cl_object x)
{
 ...
 AGAIN:
  ...
  default:
    x = ecl_type_error(ECL_SYM("RATIONAL",689),"argument",x,ECL_SYM("NUMBER",608));
    goto AGAIN;
  }
  {
#line 926
