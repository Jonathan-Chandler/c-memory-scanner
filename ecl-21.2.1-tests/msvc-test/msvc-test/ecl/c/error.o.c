#line 1 "C:/work/c/memscan-dll/ecl-21.2.1-tests/ecl-rebuild/src/c/error.d"
...
ecl_unrecoverable_error(cl_env_ptr the_env, const char *message)
{
  ...
  tag = ECL_SYM_VAL(the_env, ECL_SYM("SI::*QUIT-TAG*",1808));
  the_env->nvalues = 0;
  if (tag) {
...
FEerror(const char *s, int narg, ...)
{
  ...
  ecl_va_end(args);
  funcall(4, ECL_SYM("SI::UNIVERSAL-ERROR-HANDLER",1179),
...
CEerror(cl_object c, const char *err, int narg, ...)
{
  ...
  rest = cl_grab_rest_args(args);
  ecl_va_end(args);
  return funcall(4, ECL_SYM("SI::UNIVERSAL-ERROR-HANDLER",1179),
                 c,                       /*  correctable  */
...
FEprogram_error(const char *s, int narg, ...)
{
  ...
  if (cl_boundp(ECL_SYM("SI::*CURRENT-FORM*",1667)) != ECL_NIL) {
    /* When FEprogram_error is invoked from the compiler, we can
     * provide information about the offending form.
     */
    cl_object stmt = ecl_symbol_value(ECL_SYM("SI::*CURRENT-FORM*",1667));
    if (stmt != ECL_NIL) {
  ...
  si_signal_simple_error(4, 
                         ECL_SYM("PROGRAM-ERROR",674), /* condition name */
                         ECL_NIL, /* not correctable */
                         text,
                         real_args);
  _ecl_unexpected_return();
...
FEcontrol_error(const char *s, int narg, ...)
{
  ...
  ecl_va_end(args);
  si_signal_simple_error(4,
                         ECL_SYM("CONTROL-ERROR",257), /* condition name */
                         ECL_NIL, /* not correctable */
...
FEreader_error(const char *s, cl_object stream, int narg, ...)
{
  ...
  if (Null(stream)) {
    /* Parser error */
    si_signal_simple_error(4,
                           ECL_SYM("PARSE-ERROR",629), /* condition name */
                           ECL_NIL, /* not correctable */
    ...
    si_signal_simple_error(6,
                           ECL_SYM("READER-ERROR",701), /* condition name */
                           ECL_NIL, /* not correctable */
                           message, /* format control */
                           args_list, /* format args */
                           ECL_SYM(":STREAM",1340), stream);
  }
  _ecl_unexpected_return();
}
...
{
  cl_error(3, ECL_SYM("FILE-ERROR",355), ECL_SYM(":PATHNAME",1312), fn);
}

void
FEend_of_file(cl_object strm)
{
  cl_error(3, ECL_SYM("END-OF-FILE",331), ECL_SYM(":STREAM",1340), strm);
}

void
FEclosed_stream(cl_object strm)
{
  cl_error(3, ECL_SYM("STREAM-ERROR",803), ECL_SYM(":STREAM",1340), strm);
}

cl_object
si_signal_type_error(cl_object value, cl_object type)
{
  cl_error(5, ECL_SYM("TYPE-ERROR",872), ECL_SYM(":EXPECTED-TYPE",1254), type,
           ECL_SYM(":DATUM",1236), value);
}
              ...
              cl_index nonincl_limit)
{
  ...
  cl_error(9,
           ECL_SYM("SIMPLE-TYPE-ERROR",775), /* condition name */
           ECL_SYM(":FORMAT-CONTROL",1263), message,
           ECL_SYM(":FORMAT-ARGUMENTS",1262),
           cl_list(5, function, ecl_make_fixnum(which+1), a, ndx, type),
           ECL_SYM(":EXPECTED-TYPE",1254), type,
           ECL_SYM(":DATUM",1236), ndx);
}

void
FEunbound_variable(cl_object sym)
{
  cl_error(3, ECL_SYM("UNBOUND-VARIABLE",880), ECL_SYM(":NAME",1300), sym);
}

void
FEundefined_function(cl_object fname)
{
  cl_error(3, ECL_SYM("UNDEFINED-FUNCTION",881), ECL_SYM(":NAME",1300), fname);
}

void
FEprint_not_readable(cl_object x)
{
  cl_error(3, ECL_SYM("PRINT-NOT-READABLE",665), ECL_SYM(":OBJECT",1305), x);
}

void
FEtimeout()
{
  cl_error(1, ECL_SYM("EXT::TIMEOUT",1428));
}
...
{
  FEwrong_type_argument(ECL_SYM("FUNCTION",398), obj);
}
...
FEinvalid_function_name(cl_object fname)
{
  cl_error(9, ECL_SYM("SIMPLE-TYPE-ERROR",775), ECL_SYM(":FORMAT-CONTROL",1263),
           ...
           ECL_SYM(":FORMAT-ARGUMENTS",1262), cl_list(1, fname),
           ECL_SYM(":EXPECTED-TYPE",1254), cl_list(2, ECL_SYM("SATISFIES",737), ECL_SYM("SI::VALID-FUNCTION-NAME-P",1181)),
           ECL_SYM(":DATUM",1236), fname);
}
                        ...
                        cl_object args)
{
  ...
  if (!Null(stream)) {
    ...
    ecl_bds_bind(the_env, ECL_SYM("*PRINT-LEVEL*",53), ecl_make_fixnum(3));
    ecl_bds_bind(the_env, ECL_SYM("*PRINT-LENGTH*",52), ecl_make_fixnum(3));
    ecl_bds_bind(the_env, ECL_SYM("*PRINT-CIRCLE*",49), ECL_NIL);
...
{
  cl_error(5, ECL_SYM("DIVISION-BY-ZERO",308), ECL_SYM(":OPERATION",1307), ECL_SYM("/",23),
           ECL_SYM(":OPERANDS",1308), cl_list(2, x, y));
}
...
#line 559
	if (ecl_unlikely(narg < 1)) FEwrong_num_arguments(ecl_make_fixnum(339));
#line 559
{
  ecl_enable_interrupts();
  funcall(4, ECL_SYM("SI::UNIVERSAL-ERROR-HANDLER",1179), ECL_NIL, eformat,
...
#line 567
	if (ecl_unlikely(narg < 2)) FEwrong_num_arguments(ecl_make_fixnum(204));
#line 567
{
  ecl_enable_interrupts();
  {
#line 570
	#line 570
	cl_object __value0 = funcall(4, ECL_SYM("SI::UNIVERSAL-ERROR-HANDLER",1179), cformat, eformat,
                   cl_grab_rest_args(args));
#line 570
	the_env->nvalues = 1;
#line 570
...
#line 573
cl_object si_serror(cl_narg narg, cl_object cformat, cl_object eformat, ...)
{
#line 573
...
#line 574
	...
	ecl_va_start(args, eformat, narg, 2);
#line 574
	if (ecl_unlikely(narg < 2)) FEwrong_num_arguments(ecl_make_fixnum(1151));
#line 574
{
  ecl_enable_interrupts();
  {
#line 577
	#line 577
	cl_object __value0 = funcall(4, ECL_SYM("SI::STACK-ERROR-HANDLER",1180), cformat, eformat,
                   cl_grab_rest_args(args));
#line 577
	the_env->nvalues = 1;
#line 577
...
init_error(void)
{
  ecl_def_c_function(ECL_SYM("SI::UNIVERSAL-ERROR-HANDLER",1179),
