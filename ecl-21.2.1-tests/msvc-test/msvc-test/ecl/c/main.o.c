#line 1 "C:/work/c/memscan-dll/ecl-21.2.1-tests/ecl-rebuild/src/c/main.d"
...
maybe_fix_console_stream(cl_object stream)
{
  ...
  if (external_format == ECL_SYM(":PASS-THROUGH",1846))
    fprintf(stderr,
            ...
            GetConsoleCP());
  si_stream_external_format_set(stream, cl_list(2, external_format, ECL_SYM(":CRLF",1746)));
...
cl_boot(int argc, char **argv)
{
   ...
   */
#if 1
  ECL_SET(ECL_SYM("*DEFAULT-PATHNAME-DEFAULTS*",34), si_getcwd(0));
#else
  ECL_SET(ECL_SYM("*DEFAULT-PATHNAME-DEFAULTS*",34),
          ecl_make_pathname(ECL_NIL, ECL_NIL, ECL_NIL, ECL_NIL, ECL_NIL, ECL_NIL, ECL_SYM(":LOCAL",1294)));
#endif

#ifdef ECL_THREADS
  ECL_SET(ECL_SYM("MP::*CURRENT-PROCESS*",1432), env->own_process);
#endif
  ...
  cl_core.char_names = aux =
    cl__make_hash_table(ECL_SYM("EQUALP",338), ecl_make_fixnum(128), /* size */
                        cl_core.rehash_size,
  ...
  cl_core.system_properties =
    cl__make_hash_table(ECL_SYM("EQUAL",337), ecl_make_fixnum(1024), /* size */
                        cl_core.rehash_size,
                        cl_core.rehash_threshold);
  cl_core.setf_definitions =
    cl__make_hash_table(ECL_SYM("EQ",335), ecl_make_fixnum(256), /* size */
                        cl_core.rehash_size,
                        cl_core.rehash_threshold);

  ECL_SET(ECL_SYM("*RANDOM-STATE*",62), ecl_make_random_state(ECL_T));

  ECL_SET(ECL_SYM("FFI::C-INT-MAX",2046), ecl_make_integer(INT_MAX));
  ECL_SET(ECL_SYM("FFI::C-INT-MIN",2047), ecl_make_integer(INT_MIN));
  ECL_SET(ECL_SYM("FFI::C-LONG-MAX",2050), ecl_make_integer(LONG_MAX));
  ECL_SET(ECL_SYM("FFI::C-LONG-MIN",2051), ecl_make_integer(LONG_MIN));
  ECL_SET(ECL_SYM("FFI::C-UINT-MAX",2054), ecl_make_unsigned_integer(UINT_MAX));
  ECL_SET(ECL_SYM("FFI::C-ULONG-MAX",2056), ecl_make_unsigned_integer(ULONG_MAX));
#ifdef ecl_long_long_t
  ECL_SET(ECL_SYM("FFI::C-LONG-LONG-MAX",2052), ecl_make_long_long(LLONG_MAX));
  ECL_SET(ECL_SYM("FFI::C-ULONG-LONG-MAX",2057), ecl_make_ulong_long(ULLONG_MAX));
#endif
  ...
  ECL_SET(ECL_SYM("*PRINT-CASE*",48), ECL_SYM(":UPCASE",1350));

  /*
   * Set up hooks for LOAD, errors and macros.
   */
#ifdef ECL_THREADS
  ECL_SET(ECL_SYM("MP::+LOAD-COMPILE-LOCK+",1461),
          ecl_make_lock(ECL_SYM("MP::+LOAD-COMPILE-LOCK+",1461), 1));
#endif
#ifdef ENABLE_DLOPEN
  aux = cl_list(11,
                CONS(ECL_NIL, ECL_SYM("SI::LOAD-SOURCE",1103)),
                CONS(str_fas, ECL_SYM("SI::LOAD-BINARY",1648)),
                CONS(str_fasl, ECL_SYM("SI::LOAD-BINARY",1648)),
                CONS(str_fasb, ECL_SYM("SI::LOAD-BINARY",1648)),
                CONS(str_FASB, ECL_SYM("SI::LOAD-BINARY",1648)),
                CONS(str_lsp, ECL_SYM("SI::LOAD-SOURCE",1103)),
                CONS(str_lisp, ECL_SYM("SI::LOAD-SOURCE",1103)),
                CONS(str_LSP, ECL_SYM("SI::LOAD-SOURCE",1103)),
                CONS(str_LISP, ECL_SYM("SI::LOAD-SOURCE",1103)),
                CONS(str_fasc, ECL_SYM("SI::LOAD-BYTECODES",1922)),
                CONS(str_FASC, ECL_SYM("SI::LOAD-BYTECODES",1922)));
#else
  aux = cl_list(7,
                CONS(ECL_NIL, ECL_SYM("SI::LOAD-SOURCE",1103)),
                ...
                CONS(str_FASC, ECL_SYM("SI::LOAD-BYTECODES",1922)));
#endif
  ECL_SET(ECL_SYM("EXT::*LOAD-HOOKS*",1026), aux);
  init_error();
  init_macros();
  init_compiler();
   ...
   */
  ECL_SET(ECL_SYM("SI::*CLASS-NAME-HASH-TABLE*",1018),
          cl__make_hash_table(ECL_SYM("EQ",335), ecl_make_fixnum(1024), /* size */
                              cl_core.rehash_size,
                              cl_core.rehash_threshold));
  ...
  ECL_SET(ECL_SYM("LAMBDA-LIST-KEYWORDS",455),
          cl_list(8, ECL_SYM("&OPTIONAL",13), ECL_SYM("&REST",14), ECL_SYM("&KEY",12), ECL_SYM("&ALLOW-OTHER-KEYS",8),
                  ECL_SYM("&AUX",9), ECL_SYM("&WHOLE",15), ECL_SYM("&ENVIRONMENT",11), ECL_SYM("&BODY",10)));
  ...
  ECL_SET(ECL_SYM("*PACKAGE*",45), cl_core.lisp_package);
  ...
  if (cl_fboundp(ECL_SYM("EXT::MAKE-ENCODING",1752)) != ECL_NIL) {
  ...
  /* Jump to top level */
  ECL_SET(ECL_SYM("*PACKAGE*",45), cl_core.user_package);
  init_unixint(1);
  return 1;
}
...
#line 797
	...
	va_start(ARGS, narg);
#line 797
	if (ecl_unlikely(narg < 0|| narg > 2)) FEwrong_num_arguments(ecl_make_fixnum(1368));
#line 797
	if (narg > 0) {
#line 797
...
#endif
    ECL_SET(ECL_SYM("EXT::*PROGRAM-EXIT-CODE*",1810), code);
...
#line 828
	...
	va_start(ARGS, narg);
#line 828
	if (ecl_unlikely(narg < 0|| narg > 1)) FEwrong_num_arguments(ecl_make_fixnum(1811));
#line 828
	if (narg > 0) {
#line 828
		code = va_arg(ARGS,cl_object);
...
#line 828
		code = ECL_SYM_VAL(ecl_process_env(),ECL_SYM("EXT::*PROGRAM-EXIT-CODE*",1810));
#line 828
	}
#line 828
  cl_shutdown();
...
#line 844
	#line 844
	cl_object __value0 = ecl_make_simple_base_string(ARGV[i],-1);
#line 844
	the_env->nvalues = 1;
#line 844
	the_env->values[0] = __value0;
...
#line 858
	#line 858
	cl_object __value0 = ((value == NULL)? ECL_NIL : ecl_make_simple_base_string(value,-1));
#line 858
	the_env->nvalues = 1;
#line 858
	the_env->values[0] = __value0;
