#line 1 "C:/work/c/memscan-dll/ecl-21.2.1-tests/ecl-rebuild/src/c/compiler.d"
...
asm_end(cl_env_ptr env, cl_index beginning, cl_object definition) {
  ...
  cl_opcode *code;
  cl_object file = ECL_SYM_VAL(env,ECL_SYM("EXT::*SOURCE-LOCATION*",1202)), position;
  if (Null(file)) {
    file = ECL_SYM_VAL(env,ECL_SYM("*LOAD-TRUENAME*",40));
  ...
  bytecodes->bytecodes.name = ECL_SYM("SI::BYTECODES",1714);
...
static compiler_record database[] = {
  {ECL_SYM("BLOCK",139), c_block, 1},
  {ECL_SYM("CASE",183), c_case, 1},
  {ECL_SYM("CATCH",184), c_catch, 1},
  {ECL_SYM("EXT::COMPILER-LET",1362), c_compiler_let, 0},
  {ECL_SYM("COND",249), c_cond, 1},
  {ECL_SYM("EVAL-WHEN",342), c_eval_when, 0},
  {ECL_SYM("FLET",375), c_flet, 1},
  {ECL_SYM("FUNCTION",398), c_function, 1},
  {ECL_SYM("FUNCALL",396), c_funcall, 1},
  {ECL_SYM("GO",416), c_go, 1},
  {ECL_SYM("IF",948), c_if, 1},
  {ECL_SYM("LABELS",453), c_labels, 1},
  {ECL_SYM("LET",479), c_let, 1},
  {ECL_SYM("LET*",480), c_leta, 1},
  {ECL_SYM("LOCALLY",492), c_locally, 0},
  {ECL_SYM("LOAD-TIME-VALUE",491), c_load_time_value, 1},
  {ECL_SYM("MACROLET",523), c_macrolet, 0},
  {ECL_SYM("MULTIPLE-VALUE-BIND",574), c_multiple_value_bind, 1},
  {ECL_SYM("MULTIPLE-VALUE-CALL",575), c_multiple_value_call, 1},
  {ECL_SYM("MULTIPLE-VALUE-PROG1",577), c_multiple_value_prog1, 1},
  {ECL_SYM("MULTIPLE-VALUE-SETQ",578), c_multiple_value_setq, 1},
  {ECL_SYM("NOT",586), c_not, 1},
  {ECL_SYM("NTH-VALUE",605), c_nth_value, 1},
  {ECL_SYM("NULL",607), c_not, 1},
  {ECL_SYM("PROGN",673), compile_toplevel_body, 0},
  {ECL_SYM("PROG1",671), c_prog1, 1},
  {ECL_SYM("PROGV",675), c_progv, 1},
  {ECL_SYM("PSETQ",678), c_psetq, 1},
  {ECL_SYM("QUOTE",681), c_quote, 1},
  {ECL_SYM("RETURN",726), c_return, 1},
  {ECL_SYM("RETURN-FROM",727), c_return_from, 1},
  {ECL_SYM("SETQ",753), c_setq, 1},
  {ECL_SYM("SYMBOL-MACROLET",844), c_symbol_macrolet, 0},
  {ECL_SYM("TAGBODY",852), c_tagbody, 1},
  {ECL_SYM("THE",858), c_the, 0},
  {ECL_SYM("EXT::TRULY-THE",2039), c_the, 0},
  {ECL_SYM("THROW",860), c_throw, 1},
  {ECL_SYM("UNWIND-PROTECT",890), c_unwind_protect, 1},
  {ECL_SYM("VALUES",897), c_values, 1},
  {ECL_SYM("SI::WHILE",1530), c_while, 0},
  {ECL_SYM("EXT::WITH-BACKEND",1847), c_with_backend, 0},
  {ECL_SYM("SI::UNTIL",1531), c_until, 0},

  /* Extras */

  {ECL_SYM("CONS",253), c_cons, 1},
  {ECL_SYM("CAR",182), c_car, 1},
  {ECL_SYM("CDR",200), c_cdr, 1},
  {ECL_SYM("FIRST",373), c_car, 1},
  {ECL_SYM("REST",721), c_cdr, 1},
  {ECL_SYM("LIST",483), c_list, 1},
  {ECL_SYM("LIST*",484), c_listA, 1},
  {ECL_SYM("ENDP",332), c_endp, 1},
  {ECL_SYM("SI::CONS-CAR",1858), c_car, 1},
  {ECL_SYM("SI::CONS-CDR",1859), c_cdr, 1},
  {NULL, NULL, 1}
};
...
c_register_block(cl_env_ptr env, cl_object name)
{
  ...
  c_env->variables = CONS(cl_list(4, ECL_SYM(":BLOCK",1224), name, ECL_NIL, loc),
                          c_env->variables);
...
c_register_tags(cl_env_ptr env, cl_object all_tags)
{
  const cl_compiler_ptr c_env = env->c_env;
  cl_object loc = new_location(c_env);
  c_env->variables = CONS(cl_list(4, ECL_SYM(":TAG",1342), all_tags, ECL_NIL, loc),
                          c_env->variables);
  return ecl_fixnum(ECL_CONS_CDR(loc));
}
...
{
  const cl_compiler_ptr c_env = env->c_env;
  c_env->variables = CONS(cl_list(4, ECL_SYM(":FUNCTION",1264), name, ECL_NIL,
                                  new_location(c_env)),
                          c_env->variables);
  c_env->macros = CONS(cl_list(2, name, ECL_SYM("FUNCTION",398)), c_env->macros);
}
...
{
  const cl_compiler_ptr c_env = env->c_env;
  c_env->variables = CONS(cl_list(3, name, ECL_SYM("SI::SYMBOL-MACRO",1175), exp_fun), c_env->variables);
}
...
{
  const cl_compiler_ptr c_env = env->c_env;
  c_env->macros = CONS(cl_list(3, name, ECL_SYM("SI::MACRO",1105), exp_fun), c_env->macros);
}
...
c_register_var(cl_env_ptr env, cl_object var, bool special, bool bound)
{
  ...
  c_env->variables = CONS(cl_list(4, var,
                                  special? ECL_SYM("SPECIAL",791) : ECL_NIL,
                                  bound? ECL_T : ECL_NIL,
                                  new_location(c_env)),
                          c_env->variables);
}
...
guess_compiler_environment(cl_env_ptr env, cl_object interpreter_env)
{
   ...
   */
  for (interpreter_env = cl_revappend(interpreter_env, ECL_NIL);
       ...
       interpreter_env = ECL_CONS_CDR(interpreter_env))
    {
      ...
      if (!LISTP(record)) {
        ...
        if (ECL_SYMBOLP(record0)) {
          if (record0 == ECL_SYM("SI::MACRO",1105))
            c_register_macro(env, ECL_CONS_CDR(record1), ECL_CONS_CAR(record1));
          else if (record0 == ECL_SYM("SI::SYMBOL-MACRO",1175))
          ...
          cl_compiler_env_ptr old)
{
  ...
  if (old) {
    *new = *old;
    ...
    new->code_walker = ECL_SYM_VAL(the_env, ECL_SYM("SI::*CODE-WALKER*",1649));
    ...
    for (env = new->variables; !Null(env); env = CDR(env)) {
      ...
      if (ECL_ATOM(record))
        continue;
      if (ECL_SYMBOLP(CAR(record)) && CADR(record) != ECL_SYM("SI::SYMBOL-MACRO",1175)) {
        continue;
      } else {
        new->lexical_level = 1;
        break;
...
c_tag_ref(cl_env_ptr env, cl_object the_tag, cl_object the_type)
{
  ...
  for (l = c_env->variables; CONSP(l); l = ECL_CONS_CDR(l)) {
    ...
    if (type == ECL_SYM(":TAG",1342)) {
      ...
      n++;
    } else if (type == ECL_SYM(":BLOCK",1224) || type == ECL_SYM(":FUNCTION",1264)) {
...
c_var_ref(cl_env_ptr env, cl_object var, int allow_symbol_macro, bool ensure_defined)
{
  ...
  for (l = c_env->variables; CONSP(l); l = ECL_CONS_CDR(l)) {
    ...
    if (name == ECL_SYM(":BLOCK",1224) || name == ECL_SYM(":TAG",1342) || name == ECL_SYM(":FUNCTION",1264)) {
      n++;
    } else if (name == ECL_SYM(":DECLARE",988)) {
      /* Ignored */
    } else if (name != var) {
      /* Symbol not yet found. Only count locals. */
      if (Null(special)) n++;
    } else if (special == ECL_SYM("SI::SYMBOL-MACRO",1175)) {
  ...
  if (ensure_defined) {
    l = ecl_symbol_value(ECL_SYM("EXT::*ACTION-ON-UNDEFINED-VARIABLE*",1655));
    if (l != ECL_NIL) {
...
c_process_declarations(cl_object body)
{
  ...
  si_process_declarations(1, body);
  ...
  for (env = c_env->variables; env != old_vars && !Null(env); env = ECL_CONS_CDR(env))
    {
      ...
      record = ECL_CONS_CDR(record);
      special = ECL_CONS_CAR(record);
      if (name == ECL_SYM(":BLOCK",1224) || name == ECL_SYM(":TAG",1342)) {
        (void)0;
      } else if (name == ECL_SYM(":FUNCTION",1264) || Null(special)) {
...
c_call(cl_env_ptr env, cl_object args, int flags) {
  ...
  if (env->c_env->stepping) {
    ...
    flags = FLAG_VALUES;
             ...
             ((flags & FLAG_GLOBAL) || Null(c_tag_ref(env, name, ECL_SYM(":FUNCTION",1264)))))
  {
...
perform_c_case(cl_env_ptr env, cl_object args, int flags) {
  ...
  if (ECL_SYM("OTHERWISE",617) == test || test == ECL_T) {
...
eval_when_flags(cl_object situation)
{
  ...
  for (p = situation; p != ECL_NIL; p = ECL_CONS_CDR(p)) {
    ...
    if (keyword == ECL_SYM("LOAD",489))
      code |= FLAG_LOAD;
    else if (keyword == ECL_SYM(":LOAD-TOPLEVEL",1297))
      code |= FLAG_LOAD;
    else if (keyword == ECL_SYM("COMPILE",235))
      code |= FLAG_COMPILE;
    else if (keyword == ECL_SYM(":COMPILE-TOPLEVEL",1229))
      code |= FLAG_COMPILE;
    else if (keyword == ECL_SYM("EVAL",341))
      code |= FLAG_EXECUTE;
    else if (keyword == ECL_SYM(":EXECUTE",1253))
      code |= FLAG_EXECUTE;
  }
  return code;
}
    ...
    if (ecl_member_eq(v, fnames))
      FEprogram_error
        ("~s: The function ~s was already defined.",
         2, (op == OP_LABELS ? ECL_SYM("LABELS",453) : ECL_SYM("FLET",375)), v);
    push_back(v, f);
  }
...
create_macro_lexenv(cl_compiler_ptr c_env)
{
  ...
  for (records = c_env->macros; !Null(records); records = ECL_CONS_CDR(records)) {
    ...
    if (ECL_ATOM(record))
      continue;
    if (CADR(record) == ECL_SYM("SI::MACRO",1105))
      lexenv = CONS(CONS(ECL_SYM("SI::MACRO",1105), CONS(CADDR(record), CAR(record))), lexenv);
  }
  for (records = c_env->variables; !Null(records); records = ECL_CONS_CDR(records)) {
    ...
    if (CADR(record) == ECL_SYM("SI::SYMBOL-MACRO",1175))
      lexenv = CONS(CONS(ECL_SYM("SI::SYMBOL-MACRO",1175), CONS(CADDR(record), CAR(record))), lexenv);
  }
  return lexenv;
}
...
asm_function(cl_env_ptr env, cl_object function, int flags) {
  if (!Null(si_valid_function_name_p(function))) {
    cl_object ndx = c_tag_ref(env, function, ECL_SYM(":FUNCTION",1264));
    if (Null(ndx)) {
  ...
  if (CONSP(function)) {
    ...
    cl_object name;
    if (kind == ECL_SYM("LAMBDA",454)) {
      name = ECL_NIL;
    } else if (kind == ECL_SYM("EXT::LAMBDA-BLOCK",1367)) {
...
c_go(cl_env_ptr env, cl_object args, int flags) {
  ...
  cl_object info = c_tag_ref(env, tag, ECL_SYM(":TAG",1342));
  if (Null(info))
...
c_macrolet(cl_env_ptr the_env, cl_object args, int flags)
{
  ...
  cl_object env = funcall(3, ECL_SYM("SI::CMP-ENV-REGISTER-MACROLET",1663), pop(&args),
...
c_multiple_value_setq(cl_env_ptr env, cl_object orig_args, int flags) {
  ...
  for (orig_vars = pop(&args); !Null(orig_vars); ) {
    ...
    if (!ECL_SYMBOLP(v)) {
       ...
       */
      args = orig_args;
      return compile_form(env, cl_listX(3, ECL_SYM("SETF",752),
                                        CONS(ECL_SYM("VALUES",897), CAR(args)),
                                        CDR(args)),
                          flags);
    }
    vars = CONS(v, vars);
  ...
  if (nvars == 0) {
    /* No variables */
    return compile_form(env, cl_list(2, ECL_SYM("VALUES",897), values), flags);
  }
...
c_psetq(cl_env_ptr env, cl_object old_args, int flags) {
  ...
  */
  ...
  if (use_psetf) {
    return compile_form(env, CONS(ECL_SYM("PSETF",677), args), flags);
  }
  do {
...
{
  cl_object ndx = c_tag_ref(env, name, ECL_SYM(":BLOCK",1224));
...
c_setq(cl_env_ptr env, cl_object args, int flags) {
  ...
  do {
    ...
    if (ECL_SYMBOLP(var)) {
      ...
      compile_form(env, cl_list(3, ECL_SYM("SETF",752), var, value), flags);
    }
  } while (!Null(args));
  return flags;
}
...
c_symbol_macrolet(cl_env_ptr env, cl_object args, int flags)
{
  ...
  while (!Null(def_list)) {
    ...
    cl_object arglist = cl_list(2, cl_gensym(0), cl_gensym(0));
    cl_object function;
...
declared special and appear in a symbol-macrolet.", 1, name);
    ...
    definition = cl_list(2, arglist, cl_list(2, ECL_SYM("QUOTE",681), expansion));
...
c_unwind_protect(cl_env_ptr env, cl_object args, int flags) {
  ...
  c_register_boundary(env, ECL_SYM("SI::UNWIND-PROTECT-BOUNDARY",1919));
...
maybe_make_load_forms(cl_env_ptr env, cl_object constant)
{
  ...
  make = _ecl_funcall2(ECL_SYM("MAKE-LOAD-FORM",953), constant);
...
compile_form(cl_env_ptr env, cl_object stmt, int flags) {
  ...
  ecl_bds_bind(env, ECL_SYM("SI::*CURRENT-FORM*",1667), stmt);
 BEGIN:
  ...
  if (ECL_SYMBOLP(function)) {
    ...
    if (index != OBJNULL) {
      ...
      if (c_env->stepping && function != ECL_SYM("FUNCTION",398) &&
          c_env->lexical_level)
...
#line 2812
	...
	va_start(ARGS, body);
#line 2812
	if (ecl_unlikely(narg < 1|| narg > 2)) FEwrong_num_arguments(ecl_make_fixnum(1136));
#line 2812
	if (narg > 1) {
#line 2812
...
#line 2812
  for (; !Null(body); body = ECL_CONS_CDR(body)) {
    ...
    if (ECL_ATOM(form) || (ECL_CONS_CAR(form) != ECL_SYM("DECLARE",276))) {
      break;
    }
    for (form = ECL_CONS_CDR(form); !Null(form); ) {
      cl_object sentence = pop(&form);
      push(sentence, &declarations);
      if (pop(&sentence) == ECL_SYM("SPECIAL",791)) {
        while (!Null(sentence)) {
...
#line 2838
	#line 2838
	cl_object __value0 = cl_nreverse(declarations);
#line 2838
	cl_object __value1 = body;
#line 2838
	cl_object __value2 = documentation;
#line 2838
	cl_object __value3 = specials;
#line 2838
	the_env->nvalues = 4;
#line 2838
...
si_process_lambda(cl_object lambda)
{
  ...
  declarations = si_process_declarations(2, body, ECL_T);
  body = env->values[1];
  documentation = env->values[2];
  specials = env->values[3];

  lambda_list = si_process_lambda_list(lambda_list, ECL_SYM("FUNCTION",398));
  {
...
#define assert_var_name(v)                              \
  if (context == ECL_SYM("FUNCTION",398)) {                         \
 ...
 LOOP:
  if (Null(lambda_list))
    goto OUTPUT;
  if (!ECL_LISTP(lambda_list)) {
    unlikely_if (context == ECL_SYM("FUNCTION",398) || context == ECL_SYM("FTYPE",395))
      goto ILLEGAL_LAMBDA;
    v = lambda_list;
    lambda_list = ECL_NIL;
    goto REST;
  ...
  if (v == ECL_SYM("&OPTIONAL",13)) {
    unlikely_if (stage >= AT_OPTIONALS)
      goto ILLEGAL_LAMBDA;
    stage = AT_OPTIONALS;
    goto LOOP;
  }
  if (v == ECL_SYM("&REST",14)
      || (v == ECL_SYM("&BODY",10)
          && (context == ECL_SYM("SI::MACRO",1105) || context == ECL_SYM("DESTRUCTURING-BIND",302)))) {
  ...
  if (v == ECL_SYM("&ALLOW-OTHER-KEYS",8)) {
  ...
  case AT_REQUIREDS:
    nreq++;
    assert_var_name(v);
    if (context == ECL_SYM("FUNCTION",398) && ecl_member_eq(v, lists[0]))
  ...
  case AT_OPTIONALS: {
    ...
    if (!ECL_ATOM(v) && (context != ECL_SYM("FTYPE",395))) {
      cl_object x = v;
...
#line 3096
	#line 3096
	cl_object __value0 = CONS(ecl_make_fixnum(nreq), lists[0]);
#line 3096
	cl_object __value1 = CONS(ecl_make_fixnum(nopt), lists[1]);
#line 3096
	cl_object __value2 = rest;
#line 3096
	cl_object __value3 = key_flag;
#line 3096
	cl_object __value4 = CONS(ecl_make_fixnum(nkey), lists[2]);
#line 3096
	cl_object __value5 = allow_other_keys;
#line 3096
	cl_object __value6 = lists[3];
#line 3096
	the_env->nvalues = 7;
#line 3096
	the_env->values[6] = __value6;
#line 3096
	the_env->values[5] = __value5;
#line 3096
	the_env->values[4] = __value4;
#line 3096
	the_env->values[3] = __value3;
#line 3096
	the_env->values[2] = __value2;
...
ecl_make_lambda(cl_env_ptr env, cl_object name, cl_object lambda) {
  ...
  ecl_bds_bind(env, ECL_SYM("SI::*CURRENT-FORM*",1667),
               cl_listX(3, ECL_SYM("EXT::LAMBDA-BLOCK",1367), name, lambda));
  ...
  c_register_boundary(env, ECL_SYM("SI::FUNCTION-BOUNDARY",1918));
  ...
  if (!Null(name)) {
    compile_form(env, cl_listX(3, ECL_SYM("BLOCK",139), si_function_block_name(name),
                             body), FLAG_VALUES);
  } else {
    while (!Null(decl)) {
      cl_object l = ECL_CONS_CAR(decl);
      if (ECL_CONSP(l) && ECL_CONS_CAR(l) == ECL_SYM("SI::FUNCTION-BLOCK-NAME",1085)) {
  ...
  if (Null(ecl_symbol_value(ECL_SYM("SI::*KEEP-DEFINITIONS*",1025))))
    lambda = ECL_NIL;
...
ecl_function_block_name(cl_object name)
{
  if (ECL_SYMBOLP(name)) {
    return name;
  } else if (CONSP(name) && ECL_CONS_CAR(name) == ECL_SYM("SETF",752)) {
...
#line 3265
	#line 3265
	cl_object __value0 = (name? ECL_T : ECL_NIL);
#line 3265
	the_env->nvalues = 1;
#line 3265
	the_env->values[0] = __value0;
...
si_bc_compile_from_stream(cl_object input)
{
  ...
  ECL_UNWIND_PROTECT_BEGIN(the_env) {
    while (TRUE) {
      ...
      form = cl_read(3, input, ECL_NIL, ECL_SYM(":EOF",1250));
      if (form == ECL_SYM(":EOF",1250))
        break;
      source_location = ECL_SYM_VAL(the_env, ECL_SYM("EXT::*SOURCE-LOCATION*",1202));
...
#line 3327
	...
	va_start(ARGS, form);
#line 3327
	if (ecl_unlikely(narg < 1|| narg > 5)) FEwrong_num_arguments(ecl_make_fixnum(1070));
#line 3327
	if (narg > 1) {
#line 3327
...
#line 3327
		compiler_env_p = ECL_NIL;
#line 3327
	}
#line 3327
	if (narg > 4) {
#line 3327
		mode = va_arg(ARGS,cl_object);
#line 3327
	} else {
#line 3327
		mode = ECL_SYM(":EXECUTE",1253);
#line 3327
	}
#line 3327
  /*
   ...
   */
  if (!(mode == ECL_SYM(":EXECUTE",1253) || mode == ECL_SYM(":LOAD-TOPLEVEL",1297) || mode == ECL_SYM(":COMPILE-TOPLEVEL",1229))) {
  ...
  ECL_UNWIND_PROTECT_BEGIN(the_env) {
    if (mode == ECL_SYM(":EXECUTE",1253)) {
      eval_form(the_env, form);
    } else {
      cl_index handle = asm_begin(the_env);
      new_c_env.mode = (mode == ECL_SYM(":LOAD-TOPLEVEL",1297)) ? FLAG_LOAD : FLAG_COMPILE;
      compile_with_load_time_forms(the_env, form, FLAG_VALUES);
      asm_op(the_env, OP_EXIT);
...
init_compiler()
{
  cl_object dispatch_table =
    ...
    cl__make_hash_table(ECL_SYM("EQ",335), ecl_make_fixnum(128), /* size */
                        cl_core.rehash_size,
