#line 1 "C:/work/c/memscan-dll/ecl-21.2.1-tests/ecl-rebuild/src/c/interpreter.d"
...
too_many_arguments(register cl_object bytecodes, register cl_object frame)
{
  FEprogram_error("Too many arguments passed to "
                  ...
                  2, bytecodes, cl_apply(2, ECL_SYM("LIST",483), frame));
}
...
unknown_keyword(register cl_object bytecodes, register cl_object frame)
{
  FEprogram_error("Unknown keyword argument passed to function ~S.~&"
                  ...
                  cl_apply(2, ECL_SYM("LIST",483), frame));
}
...
ecl_interpret(cl_object frame, cl_object env, cl_object bytecodes)
{
    ...
    */
    ...
    CASE(OP_CAR); {
      if (ecl_unlikely(!LISTP(reg0)))
        FEwrong_type_only_arg(ecl_make_fixnum(/*CAR*/182), reg0, ecl_make_fixnum(/*CONS*/253));
      reg0 = CAR(reg0);
      THREAD_NEXT;
    }
    ...
    CASE(OP_CDR); {
      if (ecl_unlikely(!LISTP(reg0)))
        FEwrong_type_only_arg(ecl_make_fixnum(/*CDR*/200), reg0, ecl_make_fixnum(/*CONS*/253));
      reg0 = CDR(reg0);
      THREAD_NEXT;
    }
  ...
  DO_CALL: {
    ...
    CASE(OP_PUSHKEYS); {
      ...
      if (count) {
        if (Null(aok)) {
          ...
          cl_object *p = first;
          for (; p != last; ++p) {
            if (*(p++) == ECL_SYM(":ALLOW-OTHER-KEYS",1219)) {
              if (!Null(*p)) aok |= mask;
              mask <<= 1;
              count -= 2;
            }
    ...
    */
    ...
    CASE(OP_ENDP);
    if (ecl_unlikely(!LISTP(reg0)))
      FEwrong_type_only_arg(ecl_make_fixnum(/*ENDP*/332), reg0, ecl_make_fixnum(/*LIST*/483));
    CASE(OP_NOT); {
  ...
  PUSH_VALUES:
    ...
    */
    ...
    CASE(OP_STEPIN); {
      cl_object form;
      cl_object a = ECL_SYM_VAL(the_env, ECL_SYM("SI::*STEP-ACTION*",1034));
      cl_index n;
      ...
      if (a == ECL_T) {
         ...
         * what to do. */
        ECL_SETQ(the_env, ECL_SYM("SI::*STEP-LEVEL*",1033),
                 cl_1P(ECL_SYM_VAL(the_env, ECL_SYM("SI::*STEP-LEVEL*",1033))));
        ECL_STACK_PUSH(the_env, form);
        INTERPRET_FUNCALL(form, the_env, frame_aux, 1, ECL_SYM("SI::STEPPER",1164));
      } else if (a != ECL_NIL) {
        ...
        ECL_SETQ(the_env, ECL_SYM("SI::*STEP-ACTION*",1034), cl_1P(a));
      } else {
    ...
    CASE(OP_STEPCALL); {
      ...
      GET_OPARG(n, vector);
      SETUP_ENV(the_env);
      if (ECL_SYM_VAL(the_env, ECL_SYM("SI::*STEP-ACTION*",1034)) == ECL_T) {
        ECL_STACK_PUSH(the_env, reg0);
        INTERPRET_FUNCALL(reg0, the_env, frame_aux, 1, ECL_SYM("SI::STEPPER",1164));
      }
    ...
    CASE(OP_STEPOUT); {
      ...
      if (a == ECL_T) {
        ...
        ECL_SETQ(the_env, ECL_SYM("SI::*STEP-LEVEL*",1033),
                 cl_1M(ECL_SYM_VAL(the_env, ECL_SYM("SI::*STEP-LEVEL*",1033))));
      } else if (a == ecl_make_fixnum(0)) {
        /* We are back to the level in which the user
         * selected to step over. */
        ECL_SETQ(the_env, ECL_SYM("SI::*STEP-ACTION*",1034), ECL_T);
      } else if (a != ECL_NIL) {
        ECL_SETQ(the_env, ECL_SYM("SI::*STEP-ACTION*",1034), cl_1M(a));
      } else {
...
#endif
#line 1308
	if (ecl_unlikely(narg!=0))	   FEwrong_num_arguments(ecl_make_fixnum(1100));
#line 1308
  {
#line 1308
	#line 1308
