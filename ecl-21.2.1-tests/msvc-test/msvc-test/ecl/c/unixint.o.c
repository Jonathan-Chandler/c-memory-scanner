#line 1 "C:/work/c/memscan-dll/ecl-21.2.1-tests/ecl-rebuild/src/c/unixint.d"
...
#endif
#ifdef SIGINT
        { SIGINT, "+SIGINT+", ECL_SYM("SI::TERMINAL-INTERRUPT",1177)},
#endif
#ifdef SIGQUIT
        { SIGQUIT, "+SIGQUIT+", ECL_NIL},
#endif
#ifdef SIGILL
        { SIGILL, "+SIGILL+", ECL_SYM("EXT::ILLEGAL-INSTRUCTION",1728)},
#endif
#ifdef SIGTRAP
...
#endif
#ifdef SIGBUS
        { SIGBUS, "+SIGBUS+", ECL_SYM("EXT::SEGMENTATION-VIOLATION",1732)},
#endif
#ifdef SIGSEGV
        { SIGSEGV, "+SIGSEGV+", ECL_SYM("EXT::SEGMENTATION-VIOLATION",1732)},
#endif
#ifdef SIGSYS
...
interrupts_disabled_by_lisp(cl_env_ptr the_env)
{
        return !ecl_option_values[ECL_OPT_BOOTED] ||
                Null(ECL_SYM_VAL(the_env, ECL_SYM("EXT::*INTERRUPTS-ENABLED*",7)));
}
...
handle_signal_now(cl_object signal_code)
{
        ...
        case t_fixnum:
                cl_cerror(4, str_ignore_signal, ECL_SYM("EXT::UNIX-SIGNAL-RECEIVED",1885),
                          ECL_SYM(":CODE",1887), signal_code);
                break;
        case t_symbol:
                /*
...
handle_or_queue(cl_env_ptr the_env, cl_object signal_code, int code)
{
         ...
         */
                ...
                si_trap_fpe(ECL_SYM("LAST",457), ECL_T); /* Clear FPE exception flag */
                handle_signal_now(signal_code);
        }
}
...
asynchronous_signal_servicing_thread()
{
        ...
        for (;;) {
                ...
                signal_code = ecl_gethash_safe(ecl_make_fixnum(signal_thread_msg.signo),
                                               ...
                                               ECL_NIL);
                if (!Null(signal_code)) {
                        mp_process_run_function(3, ECL_SYM("SI::HANDLE-SIGNAL",1802),
                                                ECL_SYM("SI::HANDLE-SIGNAL",1802),
                                                signal_code);
                }
        }
# if defined(ECL_USE_MPROTECT)
...
handler_fn_prototype(fpe_signal_handler, int sig, siginfo_t *info, void *data)
{
        ...
        unlikely_if (zombie_process(the_env))
                return;
        condition = ECL_SYM("ARITHMETIC-ERROR",95);
        code = 0;
#ifdef _MSC_VER
        switch (_fpecode) {
        case _FPE_INVALID:
                condition = ECL_SYM("FLOATING-POINT-INVALID-OPERATION",382);
                code = FE_INVALID;
                break;
        case _FPE_OVERFLOW:
                condition = ECL_SYM("FLOATING-POINT-OVERFLOW",383);
                code = FE_OVERFLOW;
                break;
        case _FPE_UNDERFLOW:
                condition = ECL_SYM("FLOATING-POINT-UNDERFLOW",384);
                code = FE_UNDERFLOW;
                break;
        case _FPE_ZERODIVIDE:
                condition = ECL_SYM("DIVISION-BY-ZERO",308);
                code = FE_DIVBYZERO;
                break;
        }
#else /* !_MSC_VER */
# if defined(HAVE_FENV_H) & !defined(ECL_AVOID_FENV_H)
        ...
        if (code & FE_DIVBYZERO) {
                ...
                condition = ECL_SYM("FLOATING-POINT-INEXACT",381);
                code = FE_INEXACT;
        }
...
# endif /* ECL_DOWN_STACK */
        ...
        if (the_env->fault_address == info->si_addr) {
                ...
                the_env->fault_address = info->si_addr;
                handle_or_queue(the_env, ECL_SYM("EXT::SEGMENTATION-VIOLATION",1732), sig);
        }
#else
        /*
...
{
        if (action == ECL_NIL || action == ECL_SYM(":IGNORE",1273)) {
                mysignal(code, SIG_IGN);
                return ECL_T;
        } else if (action == ECL_SYM(":DEFAULT",1237)) {
                mysignal(code, SIG_DFL);
                return ECL_T;
        } else if (action == ECL_SYM(":MASK",1298) || action == ECL_SYM(":UNMASK",1348)) {
#ifdef HAVE_SIGPROCMASK
# ifdef ECL_THREADS
...
# endif /* !ECL_THREADS */
        ...
        } else if (action == ECL_T || action == ECL_SYM(":CATCH",1226)) {
                if (code == SIGSEGV) {
...
#endif
#line 988
	static cl_object KEYS[1] = {(cl_object)(cl_symbols+1321)};
	cl_object process;
#line 988
...
#line 988
	if (ecl_unlikely(narg < 2)) FEwrong_num_arguments(ecl_make_fixnum(1213));
#line 988
...
#line 1018
	#line 1018
	cl_object __value0 = do_catch_signal(code_int, flag, process);
#line 1018
	the_env->nvalues = 1;
#line 1018
	the_env->values[0] = __value0;
...
_ecl_w32_exception_filter(struct _EXCEPTION_POINTERS* ep)
{
        ...
        switch (ep->ExceptionRecord->ExceptionCode)
        {
                ...
                case EXCEPTION_INT_DIVIDE_BY_ZERO:
                        feclearexcept(FE_ALL_EXCEPT);
                        handle_signal_now(ECL_SYM("DIVISION-BY-ZERO",308));
                        return EXCEPTION_CONTINUE_EXECUTION;
                case EXCEPTION_INT_OVERFLOW:
                        feclearexcept(FE_ALL_EXCEPT);
                        handle_signal_now(ECL_SYM("ARITHMETIC-ERROR",95));
                        return EXCEPTION_CONTINUE_EXECUTION;
                case EXCEPTION_FLT_DIVIDE_BY_ZERO:
                        feclearexcept(FE_ALL_EXCEPT);
                        handle_signal_now(ECL_SYM("FLOATING-POINT-OVERFLOW",383));
                        return EXCEPTION_CONTINUE_EXECUTION;
                ...
                case EXCEPTION_FLT_UNDERFLOW:
                        feclearexcept(FE_ALL_EXCEPT);
                        handle_signal_now(ECL_SYM("FLOATING-POINT-UNDERFLOW",384));
                        return EXCEPTION_CONTINUE_EXECUTION;
                case EXCEPTION_FLT_INEXACT_RESULT:
                        feclearexcept(FE_ALL_EXCEPT);
                        handle_signal_now(ECL_SYM("FLOATING-POINT-INEXACT",381));
                        return EXCEPTION_CONTINUE_EXECUTION;
                case EXCEPTION_FLT_DENORMAL_OPERAND:
                case EXCEPTION_FLT_INVALID_OPERATION:
                        feclearexcept(FE_ALL_EXCEPT);
                        handle_signal_now(ECL_SYM("FLOATING-POINT-INVALID-OPERATION",382));
                        return EXCEPTION_CONTINUE_EXECUTION;
                ...
                case EXCEPTION_ACCESS_VIOLATION:
                        handle_signal_now(ECL_SYM("EXT::SEGMENTATION-VIOLATION",1732));
                        return EXCEPTION_CONTINUE_EXECUTION;
                /* Catch illegal instruction */
                case EXCEPTION_ILLEGAL_INSTRUCTION:
                        handle_signal_now(ECL_SYM("EXT::ILLEGAL-INSTRUCTION",1728));
                        return EXCEPTION_CONTINUE_EXECUTION;
...
{
        int outside_ecl = ecl_import_current_thread(ECL_SYM("SI::HANDLE-SIGNAL",1802), ECL_NIL);
        mp_process_run_function(3, ECL_SYM("SI::HANDLE-SIGNAL",1802),
                                ECL_SYM("SI::HANDLE-SIGNAL",1802),
                                signal_code);
...
BOOL WINAPI W32_console_ctrl_handler(DWORD type)
{
        ...
        case CTRL_BREAK_EVENT: {
                cl_object function =
                        ECL_SYM_FUN(ECL_SYM("SI::TERMINAL-INTERRUPT",1177));
                if (function)
        ...
        case CTRL_SHUTDOWN_EVENT: {
                cl_object function =
	                     ECL_SYM_FUN(ECL_SYM("EXT::QUIT",1368));
                if (function)
	                     W32_handle_in_new_thread(function);
                return TRUE;
        }
...
si_trap_fpe(cl_object condition, cl_object flag)
{
        ...
        int bits = 0;
        if (condition == ECL_SYM("LAST",457)) {
                bits = the_env->trap_fpe_bits;
        } else {
                if (condition == ECL_T)
                        bits = FE_DIVBYZERO | FE_OVERFLOW | FE_UNDERFLOW | FE_INVALID;
                else if (condition == ECL_SYM("DIVISION-BY-ZERO",308))
                        bits = FE_DIVBYZERO;
                else if (condition == ECL_SYM("FLOATING-POINT-OVERFLOW",383))
                        bits = FE_OVERFLOW;
                else if (condition == ECL_SYM("FLOATING-POINT-UNDERFLOW",384))
                        bits = FE_UNDERFLOW;
                else if (condition == ECL_SYM("FLOATING-POINT-INVALID-OPERATION",382))
                        bits = FE_INVALID;
                else if (condition == ECL_SYM("FLOATING-POINT-INEXACT",381))
                        bits = FE_INEXACT;
...
#line 1286
	#line 1286
	cl_object __value0 = ecl_make_fixnum(bits);
#line 1286
	the_env->nvalues = 1;
#line 1286
	the_env->values[0] = __value0;
        ...
        if (ecl_option_values[ECL_OPT_SIGNAL_HANDLING_THREAD]) {
                cl_object fun =
                        ecl_make_cfun((cl_objectfn_fixed)
                                      asynchronous_signal_servicing_thread,
                                      ECL_SYM("SI::SIGNAL-SERVICING",1844),
                                      ECL_NIL,
                                      0);
                cl_object process =
...
install_fpe_signal_handlers()
{
        ...
        if (ecl_option_values[ECL_OPT_TRAP_SIGFPE]) {
                ...
                si_trap_fpe(ECL_SYM("FLOATING-POINT-UNDERFLOW",384), ECL_NIL);

/* # if defined(ECL_IEEE_FP) */
/*                 /\* By default deactivate errors and accept denormals */
/*                  * in floating point computations. *\/ */
/*                 si_trap_fpe(ECL_SYM("FLOATING-POINT-INVALID-OPERATION",382), ECL_NIL); */
/*                 si_trap_fpe(ECL_SYM("DIVISION-BY-ZERO",308), ECL_NIL); */
/*                 si_trap_fpe(ECL_SYM("FLOATING-POINT-OVERFLOW",383), ECL_NIL); */
/* # endif */
        }
#endif
}
...
create_signal_code_constants()
{
        cl_object hash =
                cl_core.known_signals =
                cl__make_hash_table(ECL_SYM("EQL",336), ecl_make_fixnum(128),
                                    cl_core.rehash_size,
...
init_unixint(int pass)
{
        if (pass == 0) {
                ...
                install_fpe_signal_handlers();
                install_signal_handling_thread();
                ECL_SET(ECL_SYM("EXT::*INTERRUPTS-ENABLED*",7), ECL_T);
