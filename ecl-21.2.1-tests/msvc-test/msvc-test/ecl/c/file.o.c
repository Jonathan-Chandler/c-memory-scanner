#line 1 "C:/work/c/memscan-dll/ecl-21.2.1-tests/ecl-rebuild/src/c/file.d"
...
generic_read_vector(cl_object strm, cl_object data, cl_index start, cl_index end)
{
  ...
  ops = stream_dispatch_table(strm);
  if (expected_type == ECL_SYM("BASE-CHAR",122) || expected_type == ECL_SYM("CHARACTER",224)) {
...
clos_stream_read_byte8(cl_object strm, unsigned char *c, cl_index n)
{
  cl_index i;
  for (i = 0; i < n; i++) {
    cl_object byte = _ecl_funcall2(ECL_SYM("GRAY::STREAM-READ-BYTE",1689), strm);
    if (!ECL_FIXNUMP(byte))
      break;
    c[i] = ecl_fixnum(byte);
  }
...
clos_stream_write_byte8(cl_object strm, unsigned char *c, cl_index n)
{
  cl_index i;
  for (i = 0; i < n; i++) {
    cl_object byte = _ecl_funcall3(ECL_SYM("GRAY::STREAM-WRITE-BYTE",1697), strm,
                                   ecl_make_fixnum(c[i]));
    if (!ECL_FIXNUMP(byte))
      break;
  }
...
{
  cl_object b = _ecl_funcall2(ECL_SYM("GRAY::STREAM-READ-BYTE",1689), strm);
  if (b == ECL_SYM(":EOF",1250)) b = ECL_NIL;
  return b;
}
...
{
  _ecl_funcall3(ECL_SYM("GRAY::STREAM-WRITE-BYTE",1697), strm, c);
}

static ecl_character
clos_stream_read_char(cl_object strm)
{
  cl_object output = _ecl_funcall2(ECL_SYM("GRAY::STREAM-READ-CHAR",1690), strm);
  cl_fixnum value;
  if (ECL_CHARACTERP(output))
    value = ECL_CHAR_CODE(output);
  else if (ECL_FIXNUMP(output))
    value = ecl_fixnum(output);
  else if (output == ECL_NIL || output == ECL_SYM(":EOF",1250))
    return EOF;
  else
    value = -1;
...
{
  _ecl_funcall3(ECL_SYM("GRAY::STREAM-WRITE-CHAR",1698), strm, ECL_CODE_CHAR(c));
  return c;
}
...
{
  _ecl_funcall3(ECL_SYM("GRAY::STREAM-UNREAD-CHAR",1696), strm, ECL_CODE_CHAR(c));
}

static int
clos_stream_peek_char(cl_object strm)
{
  cl_object out = _ecl_funcall2(ECL_SYM("GRAY::STREAM-PEEK-CHAR",1688), strm);
  if (out == ECL_SYM(":EOF",1250)) return EOF;
...
{
  return fixnnint(_ecl_funcall5(ECL_SYM("GRAY::STREAM-READ-SEQUENCE",1693), strm, data, ecl_make_fixnum(start), ecl_make_fixnum(end)));
}

static cl_index
clos_stream_write_vector(cl_object strm, cl_object data, cl_index start, cl_index end)
{
  _ecl_funcall5(ECL_SYM("GRAY::STREAM-WRITE-SEQUENCE",1699), strm, data, ecl_make_fixnum(start), ecl_make_fixnum(end));
  if (start >= end)
    return start;
  return end;
}
...
{
  return !Null(_ecl_funcall2(ECL_SYM("GRAY::STREAM-LISTEN",1687), strm));
}

static void
clos_stream_clear_input(cl_object strm)
{
  _ecl_funcall2(ECL_SYM("GRAY::STREAM-CLEAR-INPUT",1677), strm);
}

static void
clos_stream_clear_output(cl_object strm)
{
  _ecl_funcall2(ECL_SYM("GRAY::STREAM-CLEAR-OUTPUT",1678), strm);
  return;
}
...
{
  _ecl_funcall2(ECL_SYM("GRAY::STREAM-FORCE-OUTPUT",1683), strm);
}

static void
clos_stream_finish_output(cl_object strm)
{
  _ecl_funcall2(ECL_SYM("GRAY::STREAM-FINISH-OUTPUT",1682), strm);
}

static int
clos_stream_input_p(cl_object strm)
{
  return !Null(_ecl_funcall2(ECL_SYM("GRAY::INPUT-STREAM-P",1673), strm));
}

static int
clos_stream_output_p(cl_object strm)
{
  return !Null(_ecl_funcall2(ECL_SYM("GRAY::OUTPUT-STREAM-P",1674), strm));
}

static int
clos_stream_interactive_p(cl_object strm)
{
  return !Null(_ecl_funcall2(ECL_SYM("GRAY::STREAM-INTERACTIVE-P",1685), strm));
...
{
  return _ecl_funcall2(ECL_SYM("GRAY::STREAM-ELEMENT-TYPE",1679), strm);
}
...
{
  return _ecl_funcall2(ECL_SYM("GRAY::STREAM-FILE-POSITION",1681), strm);
}

static cl_object
clos_stream_set_position(cl_object strm, cl_object pos)
{
  return _ecl_funcall3(ECL_SYM("GRAY::STREAM-FILE-POSITION",1681), strm, pos);
}

static int
clos_stream_column(cl_object strm)
{
  cl_object col = _ecl_funcall2(ECL_SYM("GRAY::STREAM-LINE-COLUMN",1686), strm);
...
{
  return _ecl_funcall2(ECL_SYM("GRAY::CLOSE",1671), strm);
}
...
#endif
#line 1501
	static cl_object KEYS[1] = {(cl_object)(cl_symbols+1246)};
...
#line 1501
	if (ecl_unlikely(narg < 0)) FEwrong_num_arguments(ecl_make_fixnum(538));
#line 1501
...
#line 1501
  if (element_type == ECL_SYM("BASE-CHAR",122)) {
    (void)0;
  } else if (element_type == ECL_SYM("CHARACTER",224)) {
#ifdef ECL_UNICODE
    extended = 1;
#endif
  } else if (!Null(_ecl_funcall3(ECL_SYM("SUBTYPEP",839), element_type, ECL_SYM("BASE-CHAR",122)))) {
    (void)0;
  } else if (!Null(_ecl_funcall3(ECL_SYM("SUBTYPEP",839), element_type, ECL_SYM("CHARACTER",224)))) {
#ifdef ECL_UNICODE
    extended = 1;
#endif
  } else {
...
cl_get_output_stream_string(cl_object strm)
{
  ...
  unlikely_if (!ECL_ANSI_STREAM_TYPE_P(strm, ecl_smm_string_output))
    FEwrong_type_only_arg(ecl_make_fixnum(/*GET-OUTPUT-STREAM-STRING*/410),
                          strm, ecl_make_fixnum(/*STRING-STREAM*/818));
...
#line 1676
	...
	va_start(ARGS, strng);
#line 1676
	if (ecl_unlikely(narg < 1|| narg > 3)) FEwrong_num_arguments(ecl_make_fixnum(537));
#line 1676
	if (narg > 1) {
#line 1676
...
#line 1676
  ...
  p = ecl_vector_start_end(ecl_make_fixnum(/*MAKE-STRING-INPUT-STREAM*/537), strng, istart, iend);
  {
#line 1678
	#line 1678
...
cl_two_way_stream_input_stream(cl_object strm)
{
  unlikely_if (!ECL_ANSI_STREAM_TYPE_P(strm,ecl_smm_two_way))
    FEwrong_type_only_arg(ecl_make_fixnum(/*TWO-WAY-STREAM-INPUT-STREAM*/869),
                          strm, ecl_make_fixnum(/*TWO-WAY-STREAM*/868));
  {
#line 1864
	const cl_env_ptr the_env = ecl_process_env();
#line 1864
...
{
  unlikely_if (!ECL_ANSI_STREAM_TYPE_P(strm, ecl_smm_two_way))
    FEwrong_type_only_arg(ecl_make_fixnum(/*TWO-WAY-STREAM-OUTPUT-STREAM*/870),
                          strm, ecl_make_fixnum(/*TWO-WAY-STREAM*/868));
  {
#line 1873
	const cl_env_ptr the_env = ecl_process_env();
...
{
  if (strm->stream.flags & ECL_STREAM_CLOSE_COMPONENTS) {
    cl_mapc(2, ECL_SYM("CLOSE",230), BROADCAST_STREAM_LIST(strm));
  }
  return generic_close(strm);
}
...
#line 2028
  ...
  x = alloc_stream();
  x->stream.format = ECL_SYM(":DEFAULT",1237);
...
cl_broadcast_stream_streams(cl_object strm)
{
  unlikely_if (!ECL_ANSI_STREAM_TYPE_P(strm, ecl_smm_broadcast))
    FEwrong_type_only_arg(ecl_make_fixnum(/*BROADCAST-STREAM-STREAMS*/162),
                          strm, ecl_make_fixnum(/*BROADCAST-STREAM*/161));
...
cl_echo_stream_input_stream(cl_object strm)
{
  unlikely_if (!ECL_ANSI_STREAM_TYPE_P(strm, ecl_smm_echo))
    FEwrong_type_only_arg(ecl_make_fixnum(/*ECHO-STREAM-INPUT-STREAM*/325),
                          strm, ecl_make_fixnum(/*ECHO-STREAM*/324));
  {
#line 2232
	const cl_env_ptr the_env = ecl_process_env();
#line 2232
...
{
  unlikely_if (!ECL_ANSI_STREAM_TYPE_P(strm, ecl_smm_echo))
    FEwrong_type_only_arg(ecl_make_fixnum(/*ECHO-STREAM-OUTPUT-STREAM*/326),
                          strm, ecl_make_fixnum(/*ECHO-STREAM*/324));
  {
#line 2241
	const cl_env_ptr the_env = ecl_process_env();
...
{
  if (strm->stream.flags & ECL_STREAM_CLOSE_COMPONENTS) {
    cl_mapc(2, ECL_SYM("CLOSE",230), CONCATENATED_STREAM_LIST(strm));
  }
  return generic_close(strm);
}
...
#line 2359
	...
	ecl_va_start(ap, narg, narg, 0);
#line 2359
	if (ecl_unlikely(narg < 0)) FEwrong_num_arguments(ecl_make_fixnum(527));
#line 2359
  streams = ECL_NIL;
  for (i = 0; i < narg; i++) {
    x = ecl_va_arg(ap);
  ...
  x = alloc_stream();
  if (Null(streams)) {
    x->stream.format = ECL_SYM(":PASS-THROUGH",1846);
  } else {
...
cl_concatenated_stream_streams(cl_object strm)
{
  unlikely_if (!ECL_ANSI_STREAM_TYPE_P(strm, ecl_smm_concatenated))
    FEwrong_type_only_arg(ecl_make_fixnum(/*CONCATENATED-STREAM-STREAMS*/248),
                          strm, ecl_make_fixnum(/*CONCATENATED-STREAM*/247));
...
cl_make_synonym_stream(cl_object sym)
{
  ...
  sym = ecl_check_cl_type(ECL_SYM("MAKE-SYNONYM-STREAM",540),sym,t_symbol);
  x = alloc_stream();
...
cl_synonym_stream_symbol(cl_object strm)
{
  unlikely_if (!ECL_ANSI_STREAM_TYPE_P(strm, ecl_smm_synonym))
    FEwrong_type_only_arg(ecl_make_fixnum(/*SYNONYM-STREAM-SYMBOL*/851),
                          strm, ecl_make_fixnum(/*SYNONYM-STREAM*/850));
  {
#line 2583
	const cl_env_ptr the_env = ecl_process_env();
#line 2583
...
io_file_read_vector(cl_object strm, cl_object data, cl_index start, cl_index end)
{
  ...
  } else if (ecl_stream_element_type(strm) == ECL_SYM("BASE-CHAR",122)
             || ecl_stream_element_type(strm) == ECL_SYM("CHARACTER",224)) {
...
{
  if (format == ECL_SYM(":DEFAULT",1237)) {
    format = ecl_symbol_value(ECL_SYM("EXT::*DEFAULT-EXTERNAL-FORMAT*",1845));
  }
  if (CONSP(format)) {
  ...
  if (format == ECL_SYM(":CR",1744)) {
    return (flags | ECL_STREAM_CR) & ~ECL_STREAM_LF;
  }
  if (format == ECL_SYM(":LF",1745)) {
    return (flags | ECL_STREAM_LF) & ~ECL_STREAM_CR;
  }
  if (format == ECL_SYM(":CRLF",1746)) {
    return flags | (ECL_STREAM_CR+ECL_STREAM_LF);
  }
  if (format == ECL_SYM(":LITTLE-ENDIAN",1755)) {
    return flags | ECL_STREAM_LITTLE_ENDIAN;
  }
  if (format == ECL_SYM(":BIG-ENDIAN",1756)) {
    return flags & ~ECL_STREAM_LITTLE_ENDIAN;
  }
  if (format == ECL_SYM(":PASS-THROUGH",1846)) {
#ifdef ECL_UNICODE
 ...
 PARSE_SYMBOLS:
  ...
  if (ECL_SYMBOLP(format)) {
    format = _ecl_funcall2(ECL_SYM("EXT::MAKE-ENCODING",1752), format);
    if (ECL_SYMBOLP(format))
      goto PARSE_SYMBOLS;
    stream->stream.format_table = format;
    return (flags & ~ECL_STREAM_FORMAT) | ECL_STREAM_USER_FORMAT;
                    ...
                    cl_object external_format)
{
  ...
  if (byte_size < 0) {
    ...
    t = ECL_SYM("SIGNED-BYTE",763);
  } else {
    flags &= ~ECL_STREAM_SIGNED_BYTES;
    t = ECL_SYM("UNSIGNED-BYTE",887);
  }
  ...
  case ECL_STREAM_LATIN_1:
    IO_STREAM_ELT_TYPE(stream) = ECL_SYM("BASE-CHAR",122);
    byte_size = 8;
    stream->stream.format = ECL_SYM(":LATIN-1",1735);
    stream->stream.encoder = passthrough_encoder;
    stream->stream.decoder = passthrough_decoder;
    break;
  case ECL_STREAM_UTF_8:
    IO_STREAM_ELT_TYPE(stream) = ECL_SYM("CHARACTER",224);
    byte_size = 8;
    stream->stream.format = ECL_SYM(":UTF-8",1737);
  ...
  case ECL_STREAM_UCS_2:
    IO_STREAM_ELT_TYPE(stream) = ECL_SYM("CHARACTER",224);
    byte_size = 8*2;
    stream->stream.format = ECL_SYM(":UCS-2",1738);
  ...
  case ECL_STREAM_UCS_2BE:
    ...
    if (flags & ECL_STREAM_LITTLE_ENDIAN) {
      stream->stream.format = ECL_SYM(":UCS-2LE",1749);
      stream->stream.encoder = ucs_2le_encoder;
      stream->stream.decoder = ucs_2le_decoder;
    } else {
      stream->stream.format = ECL_SYM(":UCS-2BE",1747);
      stream->stream.encoder = ucs_2be_encoder;
      stream->stream.decoder = ucs_2be_decoder;
    }
    break;
  case ECL_STREAM_UCS_4:
    IO_STREAM_ELT_TYPE(stream) = ECL_SYM("CHARACTER",224);
    byte_size = 8*4;
    stream->stream.format = ECL_SYM(":UCS-4BE",1748);
  ...
  case ECL_STREAM_UCS_4BE:
    ...
    byte_size = 8*4;
    if (flags & ECL_STREAM_LITTLE_ENDIAN) {
      stream->stream.format = ECL_SYM(":UCS-4LE",1750);
      stream->stream.encoder = ucs_4le_encoder;
      stream->stream.decoder = ucs_4le_decoder;
    } else {
      stream->stream.format = ECL_SYM(":UCS-4BE",1748);
  ...
  case ECL_STREAM_US_ASCII:
    IO_STREAM_ELT_TYPE(stream) = ECL_SYM("BASE-CHAR",122);
    byte_size = 8;
    stream->stream.format = ECL_SYM(":US-ASCII",1754);
  ...
  case ECL_STREAM_DEFAULT_FORMAT:
    IO_STREAM_ELT_TYPE(stream) = ECL_SYM("BASE-CHAR",122);
    byte_size = 8;
    stream->stream.format = ECL_SYM(":PASS-THROUGH",1846);
    stream->stream.encoder = passthrough_encoder;
    stream->stream.decoder = passthrough_decoder;
    break;
#endif
...
si_stream_external_format_set(cl_object stream, cl_object format)
{
  ...
  case ecl_smm_io_wcon:
#endif
    {
      ...
      unlikely_if (elt_type != ECL_SYM("CHARACTER",224) && elt_type != ECL_SYM("BASE-CHAR",122)) {
...
si_windows_codepage_encoding()
{
  ...
  default: return ECL_SYM(":PASS-THROUGH",1846);
  }
}
#else
...
si_set_buffering_mode(cl_object stream, cl_object buffer_mode_symbol)
{
  ...
  if (buffer_mode_symbol == ECL_SYM(":NONE",1658) || Null(buffer_mode_symbol))
    buffer_mode = _IONBF;
  else if (buffer_mode_symbol == ECL_SYM(":LINE",1661) || buffer_mode_symbol == ECL_SYM(":LINE-BUFFERED",1659))
    buffer_mode = _IOLBF;
  else if (buffer_mode_symbol == ECL_SYM(":FULL",1662) || buffer_mode_symbol == ECL_SYM(":FULLY-BUFFERED",1660))
    buffer_mode = _IOFBF;
  else
...
#endif
#line 4574
	static cl_object KEYS[3] = {(cl_object)(cl_symbols+1337), (cl_object)(cl_symbols+1247), (cl_object)(cl_symbols+1258)};
	cl_object start;
	cl_object end;
	cl_object external_format;
#line 4574
...
#line 4574
	if (ecl_unlikely(narg < 1)) FEwrong_num_arguments(ecl_make_fixnum(2012));
#line 4574
...
#line 4574
  p = ecl_vector_start_end(ecl_make_fixnum(/*EXT::MAKE-SEQUENCE-INPUT-STREAM*/2012),
                           vector, start, end);
  {
#line 4577
	#line 4577
 ...
 AGAIN:
  {
    ...
    if (delta < n) {
      ...
      vector = _ecl_funcall3(ECL_SYM("ADJUST-ARRAY",85), vector,
...
#endif
#line 4700
	static cl_object KEYS[1] = {(cl_object)(cl_symbols+1258)};
	cl_object external_format;
#line 4700
	cl_object KEY_VARS[2];
#line 4700
	...
	ecl_va_start(ARGS, vector, narg, 1);
#line 4700
	if (ecl_unlikely(narg < 1)) FEwrong_num_arguments(ecl_make_fixnum(2013));
#line 4700
	cl_parse_key(ARGS, 1, KEYS, KEY_VARS, NULL, 0);
#line 4700
	if (KEY_VARS[1]==ECL_NIL) {
...
stream_dispatch_table(cl_object strm)
{
  ...
  if (!ECL_ANSI_STREAM_P(strm))
    FEwrong_type_argument(ecl_make_fixnum(/*STREAM*/801), strm);
...
#endif
  unlikely_if (!ECL_ANSI_STREAM_P(stream)) {
    FEwrong_type_only_arg(ecl_make_fixnum(/*FILE-STRING-LENGTH*/361), stream, ecl_make_fixnum(/*STREAM*/801));
  }
...
;
  ...
  case t_character:
    ...
    break;
  default:
    FEwrong_type_nth_arg(ecl_make_fixnum(/*FILE-STRING-LENGTH*/361), 2, string, ecl_make_fixnum(/*STRING*/807));
  }
  {
#line 4962
	const cl_env_ptr the_env = ecl_process_env();
...
si_do_write_sequence(cl_object seq, cl_object stream, cl_object s, cl_object e)
{
  ...
  if (ecl_unlikely(!ECL_FIXNUMP(s) ||
                   ...
                   (start > limit))) {
    FEwrong_type_key_arg(ecl_make_fixnum(/*WRITE-SEQUENCE*/923), ecl_make_fixnum(/*:START*/1337), s,
  ...
  if (e == ECL_NIL) {
                          ...
                          ((end = ecl_fixnum(e)) < 0) ||
                          (end > limit))) {
    FEwrong_type_key_arg(ecl_make_fixnum(/*WRITE-SEQUENCE*/923), ecl_make_fixnum(/*:END*/1247), e,
                         ecl_make_integer_type(ecl_make_fixnum(0),
  ...
  if (LISTP(seq)) {
    ...
    bool ischar = (elt_type == ECL_SYM("BASE-CHAR",122)) || (elt_type == ECL_SYM("CHARACTER",224));
...
si_do_read_sequence(cl_object seq, cl_object stream, cl_object s, cl_object e)
{
  ...
  if (ecl_unlikely(!ECL_FIXNUMP(s) ||
                   ((start = ecl_fixnum(s)) < 0) ||
                   (start > limit))) {
    FEwrong_type_key_arg(ecl_make_fixnum(/*READ-SEQUENCE*/700), ecl_make_fixnum(/*:START*/1337), s,
                         ecl_make_integer_type(ecl_make_fixnum(0),
                                               ecl_make_fixnum(limit-1)));
  }
  if (e == ECL_NIL) {
                          ...
                          ((end = ecl_fixnum(e)) < 0) ||
                          (end > limit))) {
    FEwrong_type_key_arg(ecl_make_fixnum(/*READ-SEQUENCE*/700), ecl_make_fixnum(/*:END*/1247), e,
                         ecl_make_integer_type(ecl_make_fixnum(0),
                                               ecl_make_fixnum(limit)));
  }
  if (end <= start) {
...
#line 5084
	#line 5084
	cl_object __value0 = (column >= 0 ? ecl_make_fixnum(column) : ECL_NIL);
#line 5084
	the_env->nvalues = 1;
#line 5084
	the_env->values[0] = __value0;
...
#line 5096
	if (ecl_unlikely(narg < 1|| narg > 2)) FEwrong_num_arguments(ecl_make_fixnum(359));
#line 5096
	if (narg > 1) {
#line 5096
...
#line 5096
  if (Null(position)) {
    ...
    if (position == ECL_SYM(":START",1337)) {
      position = ecl_make_fixnum(0);
    } else if (position == ECL_SYM(":END",1247)) {
      position = ECL_NIL;
    }
...
#ifdef ECL_CLOS_STREAMS
  if (ECL_INSTANCEP(strm)) {
    return _ecl_funcall2(ECL_SYM("GRAY::OPEN-STREAM-P",1675), strm);
  }
#endif
  unlikely_if (!ECL_ANSI_STREAM_P(strm))
    FEwrong_type_only_arg(ECL_SYM("OPEN-STREAM-P",614), strm, ECL_SYM("STREAM",801));
  {
#line 5140
	const cl_env_ptr the_env = ecl_process_env();
#line 5140
...
#ifdef ECL_CLOS_STREAMS
  if (t == t_instance)
    output = ECL_SYM(":DEFAULT",1237);
  else
#endif
    unlikely_if (t != t_stream)
      FEwrong_type_only_arg(ecl_make_fixnum(/*STREAM-EXTERNAL-FORMAT*/805), strm, ecl_make_fixnum(/*STREAM*/801));
...
#ifdef ECL_CLOS_STREAMS
  if (ECL_INSTANCEP(strm)) {
    return _ecl_funcall2(ECL_SYM("GRAY::STREAMP",1672), strm);
  }
#endif
  {
#line 5179
...
#line 5200
	#line 5200
	cl_object __value0 = ((c==EOF) ? ECL_T : ECL_NIL);
#line 5200
	the_env->nvalues = 1;
#line 5200
	the_env->values[0] = __value0;
...
ecl_normalize_stream_element_type(cl_object element_type)
{
  cl_fixnum sign = 0;
  cl_index size;
  if (element_type == ECL_SYM("SIGNED-BYTE",763) || element_type == ECL_SYM("EXT::INTEGER8",1363)) {
    return -8;
  } else if (element_type == ECL_SYM("UNSIGNED-BYTE",887) || element_type == ECL_SYM("EXT::BYTE8",1358)) {
    return 8;
  } else if (element_type == ECL_SYM(":DEFAULT",1237)) {
    return 0;
  } else if (element_type == ECL_SYM("BASE-CHAR",122) || element_type == ECL_SYM("CHARACTER",224)) {
    return 0;
  } else if (_ecl_funcall3(ECL_SYM("SUBTYPEP",839), element_type, ECL_SYM("CHARACTER",224)) != ECL_NIL) {
    return 0;
  } else if (_ecl_funcall3(ECL_SYM("SUBTYPEP",839), element_type, ECL_SYM("UNSIGNED-BYTE",887)) != ECL_NIL) {
    sign = +1;
  } else if (_ecl_funcall3(ECL_SYM("SUBTYPEP",839), element_type, ECL_SYM("SIGNED-BYTE",763)) != ECL_NIL) {
    sign = -1;
  } else {
    FEerror("Not a valid stream element type: ~A", 1, element_type);
  }
  if (CONSP(element_type)) {
    if (CAR(element_type) == ECL_SYM("UNSIGNED-BYTE",887))
      return ecl_to_size(cl_cadr(element_type));
    if (CAR(element_type) == ECL_SYM("SIGNED-BYTE",763))
      return -ecl_to_size(cl_cadr(element_type));
  }
  for (size = 8; 1; size++) {
    cl_object type;
    type = cl_list(2, sign>0? ECL_SYM("UNSIGNED-BYTE",887) : ECL_SYM("SIGNED-BYTE",763),
                   ecl_make_fixnum(size));
    if (_ecl_funcall3(ECL_SYM("SUBTYPEP",839), element_type, type) != ECL_NIL) {
      return size * sign;
    }
  }
  FEerror("Not a valid stream element type: ~A", 1, element_type);
                ...
                int flags, cl_object external_format)
{
  ...
  if (!exists) {
    ...
    if (if_does_not_exist == ECL_SYM(":ERROR",1251)) FEcannot_open(fn);
    if (if_does_not_exist != ECL_SYM(":CREATE",1235))
      FEinvalid_option(ECL_SYM(":IF-DOES-NOT-EXIST",1268), if_does_not_exist);
  ...
  case ecl_smm_io: {
    if (exists) {
      ...
      if (if_exists == ECL_SYM(":ERROR",1251)) FEcannot_open(fn);
      if (if_exists == ECL_SYM(":RENAME",1327)) {
        fd = ecl_backup_open(fname, O_RDWR|O_CREAT, mode);
        unlikely_if (fd < 0) FEcannot_open(fn);
      } else if (if_exists == ECL_SYM(":RENAME-AND-DELETE",1328) ||
                 if_exists == ECL_SYM(":NEW-VERSION",1302) ||
                 if_exists == ECL_SYM(":SUPERSEDE",1341) ||
                 if_exists == ECL_SYM(":TRUNCATE",1345)) {
        fd = safe_open(fname, O_RDWR|O_TRUNC, mode);
        unlikely_if (fd < 0) FEcannot_open(fn);
      } else if (if_exists == ECL_SYM(":OVERWRITE",1310) || if_exists == ECL_SYM(":APPEND",1220)) {
        fd = safe_open(fname, O_RDWR, mode);
        unlikely_if (fd < 0) FEcannot_open(fn);
        appending = (if_exists == ECL_SYM(":APPEND",1220));
      } else {
        FEinvalid_option(ECL_SYM(":IF-EXISTS",1270), if_exists);
      }
    }
    break;
  }
  ...
  if (flags & ECL_STREAM_C_STREAM) {
    ...
    case ecl_smm_input:
      if (file_kind == ECL_SYM(":FIFO",1259)) {
    ...
    si_set_buffering_mode(output, byte_size? ECL_SYM(":FULL",1662) : ECL_SYM(":LINE",1661));
  } else {
...
#endif
#line 5374
	static cl_object KEYS[6] = {(cl_object)(cl_symbols+1240), (cl_object)(cl_symbols+1246), (cl_object)(cl_symbols+1270), (cl_object)(cl_symbols+1268), (cl_object)(cl_symbols+1258), (cl_object)(cl_symbols+1352)};
	cl_object direction;
	cl_object element_type;
	cl_object if_exists;
	bool iesp;
...
#line 5374
	if (ecl_unlikely(narg < 1)) FEwrong_num_arguments(ecl_make_fixnum(613));
#line 5374
...
#line 5374
  ...
  if (direction == ECL_SYM(":INPUT",1280)) {
    ...
    if (!iesp)
      if_exists = ECL_SYM(":NEW-VERSION",1302);
    if (!idnesp) {
      if (if_exists == ECL_SYM(":OVERWRITE",1310) ||
          if_exists == ECL_SYM(":APPEND",1220))
        if_does_not_exist = ECL_SYM(":ERROR",1251);
      else
        if_does_not_exist = ECL_SYM(":CREATE",1235);
    }
  ...
  } else if (direction == ECL_SYM(":PROBE",1320)) {
    smm = ecl_smm_probe;
    if (!idnesp)
      if_does_not_exist = ECL_NIL;
  } else {
...
#endif
#line 5423
	static cl_object KEYS[1] = {(cl_object)(cl_symbols+1217)};
	cl_object abort;
#line 5423
	cl_object KEY_VARS[2];
#line 5423
...
#line 5423
	if (ecl_unlikely(narg < 1)) FEwrong_num_arguments(ecl_make_fixnum(230));
#line 5423
	cl_parse_key(ARGS, 1, KEYS, KEY_VARS, NULL, 0);
#line 5423
	if (KEY_VARS[1]==ECL_NIL) {
...
# if defined(HAVE_SELECT)
  ...
  if (ecl_unlikely(retv < 0))
    file_libc_error(ecl_make_fixnum(/*STREAM-ERROR*/803), stream, "Error while listening to stream.", 0);
...
flisten(cl_object stream, FILE *fp)
{
     ...
     It will fail on noninteractive streams. */
  {
    ...
    unlikely_if (old_pos == -1 ||
      ...
      file_libc_error(ecl_make_fixnum(/*FILE-ERROR*/355), stream,
...
not_a_file_stream(cl_object strm)
{
  cl_error(9, ECL_SYM("SIMPLE-TYPE-ERROR",775), ECL_SYM(":FORMAT-CONTROL",1263),
           ...
           ECL_SYM(":FORMAT-ARGUMENTS",1262), cl_list(1, strm),
           ECL_SYM(":EXPECTED-TYPE",1254), ECL_SYM("FILE-STREAM",360),
           ECL_SYM(":DATUM",1236), strm);
}
...
not_an_input_stream(cl_object strm)
{
  cl_error(9, ECL_SYM("SIMPLE-TYPE-ERROR",775), ECL_SYM(":FORMAT-CONTROL",1263),
           ...
           ECL_SYM(":FORMAT-ARGUMENTS",1262), cl_list(1, strm),
           ECL_SYM(":EXPECTED-TYPE",1254),
           cl_list(2, ECL_SYM("SATISFIES",737), ECL_SYM("INPUT-STREAM-P",437)),
           ECL_SYM(":DATUM",1236), strm);
}
...
not_an_output_stream(cl_object strm)
{
  cl_error(9, ECL_SYM("SIMPLE-TYPE-ERROR",775), ECL_SYM(":FORMAT-CONTROL",1263),
           ...
           ECL_SYM(":FORMAT-ARGUMENTS",1262), cl_list(1, strm),
           ECL_SYM(":EXPECTED-TYPE",1254), cl_list(2, ECL_SYM("SATISFIES",737), ECL_SYM("OUTPUT-STREAM-P",618)),
           ECL_SYM(":DATUM",1236), strm);
}
...
not_a_character_stream(cl_object s)
{
  cl_error(9, ECL_SYM("SIMPLE-TYPE-ERROR",775), ECL_SYM(":FORMAT-CONTROL",1263),
           ...
           ECL_SYM(":FORMAT-ARGUMENTS",1262), cl_list(1, s),
           ECL_SYM(":EXPECTED-TYPE",1254), ECL_SYM("CHARACTER",224),
           ECL_SYM(":DATUM",1236), cl_stream_element_type(s));
}
...
not_a_binary_stream(cl_object s)
{
  cl_error(9, ECL_SYM("SIMPLE-TYPE-ERROR",775), ECL_SYM(":FORMAT-CONTROL",1263),
           ...
           ECL_SYM(":FORMAT-ARGUMENTS",1262), cl_list(1, s),
           ECL_SYM(":EXPECTED-TYPE",1254), ECL_SYM("INTEGER",439),
           ECL_SYM(":DATUM",1236), cl_stream_element_type(s));
}
...
{
  file_libc_error(ecl_make_fixnum(/*FILE-ERROR*/355), stream, "Stream cannot be closed", 0);
}
...
restartable_io_error(cl_object strm, const char *s)
{
  ...
  if (old_errno == EINTR) {
    ...
    file_libc_error(ecl_make_fixnum(/*STREAM-ERROR*/803), strm,
...
{
  cl_object code = _ecl_funcall4(ECL_SYM("EXT::ENCODING-ERROR",1914), stream,
...
decoding_error(cl_object stream, unsigned char **buffer, int char_length, unsigned char *buffer_end)
{
  ...
  code = _ecl_funcall4(ECL_SYM("EXT::DECODING-ERROR",1913), stream,
                       cl_stream_external_format(stream),
                       octets);
  if (Null(code)) {
...
init_file(void)
{
  ...
  external_format = cl_list(2, ECL_SYM(":PASS-THROUGH",1846), ECL_SYM(":CRLF",1746));
# ifdef ECL_UNICODE
  flags = 0;
# else
...
# endif
  ...
  ECL_SET(ECL_SYM("EXT::+PROCESS-STANDARD-INPUT+",1894), standard_input);
  ECL_SET(ECL_SYM("*STANDARD-INPUT*",68), standard_input);
  cl_core.standard_output = standard_output;
  ECL_SET(ECL_SYM("EXT::+PROCESS-STANDARD-OUTPUT+",1895), standard_output);
  ECL_SET(ECL_SYM("*STANDARD-OUTPUT*",69), standard_output);
  ECL_SET(ECL_SYM("*TRACE-OUTPUT*",71), standard_output);
  cl_core.error_output = error_output;
  ECL_SET(ECL_SYM("EXT::+PROCESS-ERROR-OUTPUT+",1896), error_output);
  ECL_SET(ECL_SYM("*ERROR-OUTPUT*",35), error_output);

  cl_core.terminal_io = aux 
    = cl_make_two_way_stream(standard_input, standard_output);

  ECL_SET(ECL_SYM("*TERMINAL-IO*",70), aux);
  aux = cl_make_synonym_stream(ECL_SYM("*TERMINAL-IO*",70));
  ECL_SET(ECL_SYM("*QUERY-IO*",61), aux);
  ECL_SET(ECL_SYM("*DEBUG-IO*",32), aux);
}
