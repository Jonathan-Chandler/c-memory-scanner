#line 1 "C:/work/c/memscan-dll/ecl-21.2.1-tests/ecl-rebuild/src/c/numbers/number_compare.d"
...
ecl_number_compare(cl_object x, cl_object y)
{
 ...
 BEGIN:
  MATH_DISPATCH2_BEGIN(x,y) {
    ...
    CASE_UNKNOWN(ecl_make_fixnum(/*<*/74),x,y,ecl_make_fixnum(/*REAL*/705));
  }
  MATH_DISPATCH2_END;
}
...
monotonic(int s, int t, int narg, ecl_va_list nums)
{
  ...
  if (narg == 1) {
    ...
    FEwrong_type_nth_arg(ecl_make_fixnum(/*<*/74), 1, c, ecl_make_fixnum(/*REAL*/705));
  }
...
cl_object cl_LE MONOTONIC( 1, 0)
cl_object cl_GE MONOTONIC(-1, 0)
cl_object cl_L  MONOTONIC( 1, 1)
cl_object cl_G  MONOTONIC(-1, 1)
