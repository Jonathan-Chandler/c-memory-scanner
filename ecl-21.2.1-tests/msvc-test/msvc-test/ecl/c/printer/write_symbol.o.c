#line 1 "C:/work/c/memscan-dll/ecl-21.2.1-tests/ecl-rebuild/src/c/printer/write_symbol.d"
      ...
      if (ecl_upper_case_p(c)) {
        if ((action == ecl_case_invert) ||
            ((action == ecl_case_upcase) &&
             ((print_case == ECL_SYM(":DOWNCASE",1245)) ||
              ((print_case == ECL_SYM(":CAPITALIZE",1225)) && !capitalize))))
          {
        ...
        if ((action == ecl_case_invert) ||
            ((action == ecl_case_downcase) &&
             ((print_case == ECL_SYM(":UPCASE",1350)) ||
              ((print_case == ECL_SYM(":CAPITALIZE",1225)) && capitalize))))
          {
            c = ecl_char_upcase(c);
          }
...
{
  cl_object print_package = ecl_symbol_value(ECL_SYM("SI::*PRINT-PACKAGE*",1030));
