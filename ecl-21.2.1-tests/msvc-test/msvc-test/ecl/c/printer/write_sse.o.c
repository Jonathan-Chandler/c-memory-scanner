#line 1 "C:/work/c/memscan-dll/ecl-21.2.1-tests/ecl-rebuild/src/c/printer/write_sse.d"
...
write_sse_pack(cl_object x, cl_object stream)
{
  ...
  cl_object mode = ecl_symbol_value(ECL_SYM("EXT::*SSE-PACK-PRINT-MODE*",1884));

  if (mode != ECL_NIL) {
    if (mode == ECL_SYM(":FLOAT",1400)) etype = ecl_aet_sf;
    else if (mode == ECL_SYM(":DOUBLE",1398)) etype = ecl_aet_df;
