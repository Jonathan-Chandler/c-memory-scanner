#line 1 "C:/work/c/memscan-dll/ecl-21.2.1-tests/ecl-rebuild/src/c/printer/write_array.d"
...
write_array_inner(bool vector, cl_object x, cl_object stream)
{
  ...
  if (print_level >= n) {
    ...
    print_level -= n;
    ecl_bds_bind(env, ECL_SYM("*PRINT-LEVEL*",53), ecl_make_fixnum(print_level));
  } else {
