#line 1 "C:/work/c/memscan-dll/ecl-21.2.1-tests/ecl-rebuild/src/c/printer/write_ugly.d"
...
{
  cl_object l =
    cl_list(15, ECL_SYM("MAKE-PATHNAME",533),
            ECL_SYM(":HOST",1267), path->pathname.host,
            ECL_SYM(":DEVICE",1239), path->pathname.device,
            ECL_SYM(":DIRECTORY",1241),
            _ecl_funcall2(ECL_SYM("EXT::MAYBE-QUOTE",1742), path->pathname.directory),
            ECL_SYM(":NAME",1300), path->pathname.name,
            ECL_SYM(":TYPE",1346), path->pathname.type,
            ECL_SYM(":VERSION",1354), path->pathname.version,
            ECL_SYM(":DEFAULTS",1238), ECL_NIL);
...
write_integer(cl_object number, cl_object stream)
{
  ...
  si_integer_to_string(s, number,
                       ...
                       ecl_symbol_value(ECL_SYM("*PRINT-RADIX*",58)),
...
{
  if (ecl_print_readably() && !Null(ecl_symbol_value(ECL_SYM("*READ-EVAL*",65)))) {
    cl_object make =
      cl_list(15, ECL_SYM("MAKE-HASH-TABLE",530),
              ECL_SYM(":SIZE",1335), cl_hash_table_size(x),
              ECL_SYM(":SYNCHRONIZED",2037), si_hash_table_synchronized_p(x),
              ECL_SYM(":WEAKNESS",2035), si_hash_table_weakness(x),
              ECL_SYM(":HASH-FUNCTION",1266), x->hash.generic_hash,
              ECL_SYM(":REHASH-SIZE",1324), cl_hash_table_rehash_size(x),
              ECL_SYM(":REHASH-THRESHOLD",1325), cl_hash_table_rehash_threshold(x),
              ECL_SYM(":TEST",1343), cl_list(2, ECL_SYM("QUOTE",681), cl_hash_table_test(x)));
    cl_object init =
      cl_list(3, ECL_SYM("EXT::HASH-TABLE-FILL",2017), make,
              cl_list(2, ECL_SYM("QUOTE",681), si_hash_table_content(x)));
    writestr_stream("#.", stream);
...
{
  _ecl_funcall3(ECL_SYM("PRINT-OBJECT",963), x, stream);
}
