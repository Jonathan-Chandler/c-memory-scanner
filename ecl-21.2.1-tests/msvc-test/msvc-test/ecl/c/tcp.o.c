#line 1 "C:/work/c/memscan-dll/ecl-21.2.1-tests/ecl-rebuild/src/c/tcp.d"
...
si_open_client_stream(cl_object host, cl_object port)
{
  ...
  if (ecl_unlikely(!ECL_FIXNUMP(port) ||
    ...
    FEwrong_type_nth_arg(ecl_make_fixnum(/*SI::OPEN-CLIENT-STREAM*/1209), 2, port,
...
si_open_server_stream(cl_object port)
{
  ...
  if (ecl_unlikely(!ECL_FIXNUMP(port) ||
                   ecl_fixnum_minusp(port) ||
                   ecl_fixnum_greater(port,ecl_make_fixnum(65535)))) {
    FEwrong_type_only_arg(ecl_make_fixnum(/*SI::OPEN-SERVER-STREAM*/1210), port,
                          ecl_read_from_cstring("(INTEGER 0 65535)"));
  }
  p = ecl_fixnum(port);
  ecl_disable_interrupts();
...
si_open_unix_socket_stream(cl_object path)
{
  ...
  if (ecl_unlikely(!ECL_STRINGP(path)))
    FEwrong_type_only_arg(ecl_make_fixnum(/*SI::OPEN-UNIX-SOCKET-STREAM*/1211), path, ecl_make_fixnum(/*STRING*/807));
