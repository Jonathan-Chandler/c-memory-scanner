#line 1 "C:/work/c/memscan-dll/ecl-21.2.1-tests/ecl-rebuild/src/c/array.d"
...
static const cl_object ecl_aet_name[] = {
  ...
  ECL_SYM("SINGLE-FLOAT",780),      /* ecl_aet_sf */
  ECL_SYM("DOUBLE-FLOAT",317),      /* ecl_aet_df */
  ECL_SYM("LONG-FLOAT",510),        /* ecl_aet_lf */
#ifdef ECL_COMPLEX_FLOAT
  ECL_SYM("SI::COMPLEX-SINGLE-FLOAT",1872),  /* ecl_aet_csf */
  ECL_SYM("SI::COMPLEX-DOUBLE-FLOAT",1873),  /* ecl_aet_cdf */
  ECL_SYM("SI::COMPLEX-LONG-FLOAT",1874),    /* ecl_aet_clf */
#endif
  ECL_SYM("BIT",125),               /* ecl_aet_bit */
  ECL_SYM("EXT::CL-FIXNUM",1643),    /* ecl_aet_fix */
  ECL_SYM("EXT::CL-INDEX",1644),     /* ecl_aet_index */
  ECL_SYM("EXT::BYTE8",1358),        /* ecl_aet_b8 */
  ECL_SYM("EXT::INTEGER8",1363),     /* ecl_aet_i8 */
#ifdef ecl_uint16_t
  ECL_SYM("EXT::BYTE16",1359),
  ECL_SYM("EXT::INTEGER16",1364),
#endif
#ifdef ecl_uint32_t
  ECL_SYM("EXT::BYTE32",1360),
  ECL_SYM("EXT::INTEGER32",1365),
#endif
#ifdef ecl_uint64_t
  ECL_SYM("EXT::BYTE64",1361),
  ECL_SYM("EXT::INTEGER64",1366),
#endif
#ifdef ECL_UNICODE
  ECL_SYM("CHARACTER",224),         /* ecl_aet_ch */
#endif
  ECL_SYM("BASE-CHAR",122)          /* ecl_aet_bc */
};
...
{
  cl_object type = cl_list(3, ECL_SYM("INTEGER",439), ecl_make_fixnum(0),
...
FEwrong_dimensions(cl_object a, cl_index rank)
{
  cl_object list = cl_make_list(3, ecl_make_fixnum(rank),
                                ECL_SYM(":INITIAL-ELEMENT",1277), ECL_SYM("*",20));
  cl_object type = cl_list(3, ECL_SYM("ARRAY",98), ECL_SYM("*",20), list);
...
ecl_to_index(cl_object n)
{
  ...
  default:
    FEwrong_type_only_arg(ecl_make_fixnum(/*COERCE*/233), n, ecl_make_fixnum(/*FIXNUM*/374));
  }
}
...
#line 115
	#line 115
	cl_object __value0 = ecl_aref(x, j);
#line 115
	the_env->nvalues = 1;
#line 115
...
#line 122
	#line 122
	cl_object __value0 = ecl_aset(x, j, val);
#line 122
	the_env->nvalues = 1;
#line 122
	the_env->values[0] = __value0;
...
#line 126
	if (ecl_unlikely(narg < 1)) FEwrong_num_arguments(ecl_make_fixnum(94));
#line 126
{
  cl_index i, j;
  cl_index r = narg - 1;
  ...
  case t_array:
    ...
    for (i = j = 0;  i < r;  i++) {
      cl_index s = checked_index(ecl_make_fixnum(/*AREF*/94), x, i,
                                 ecl_va_arg(indx),
                                 x->array.dims[i]);
  ...
  case t_bitvector:
    if (r != 1)
      FEerror("Wrong number of indices.", 0);
    j = checked_index(ecl_make_fixnum(/*AREF*/94), x, -1, ecl_va_arg(indx), x->vector.dim);
    break;
  default:
    FEwrong_type_nth_arg(ecl_make_fixnum(/*AREF*/94), 1, x, ecl_make_fixnum(/*ARRAY*/98));
  }
  {
#line 153
	#line 153
	cl_object __value0 = ecl_aref_unsafe(x, j);
#line 153
	the_env->nvalues = 1;
#line 153
	the_env->values[0] = __value0;
...
ecl_aref(cl_object x, cl_index index)
{
  ...
  if (ecl_unlikely(index >= x->array.dim)) {
    FEwrong_index(ecl_make_fixnum(/*ROW-MAJOR-AREF*/733), x, -1, ecl_make_fixnum(index),
                  x->array.dim);
  }
...
ecl_aref1(cl_object x, cl_index index)
{
  ...
  if (ecl_unlikely(index >= x->array.dim)) {
    FEwrong_index(ecl_make_fixnum(/*AREF*/94), x, -1, ecl_make_fixnum(index),
                  x->array.dim);
  }
  return ecl_aref_unsafe(x, index);
}
...
#line 278
	if (ecl_unlikely(narg < 1)) FEwrong_num_arguments(ecl_make_fixnum(1042));
#line 278
{
    cl_index i, j;
    cl_index r = narg - 2;
    ...
    case t_array:
      if (ecl_unlikely(r != x->array.rank))
        FEerror("Wrong number of indices.", 0);
      for (i = j = 0;  i < r;  i++) {
        cl_index s = checked_index(ecl_make_fixnum(/*SI::ASET*/1042), x, i,
                                   ecl_va_arg(dims),
                                   x->array.dims[i]);
        j = j*(x->array.dims[i]) + s;
      }
    ...
    case t_bitvector:
      if (ecl_unlikely(r != 1))
        FEerror("Wrong number of indices.", 0);
      j = checked_index(ecl_make_fixnum(/*SI::ASET*/1042), x, -1, ecl_va_arg(dims),
                        x->vector.dim);
      break;
    default:
      FEwrong_type_nth_arg(ecl_make_fixnum(/*SI::ASET*/1042), 1, x, ecl_make_fixnum(/*ARRAY*/98));
    }
    v = ecl_va_arg(dims);
    {
#line 308
	#line 308
	cl_object __value0 = ecl_aset_unsafe(x, j, v);
#line 308
	the_env->nvalues = 1;
#line 308
	the_env->values[0] = __value0;
                   ...
                   cl_object fillp, cl_object displ, cl_object disploff)
{
  ...
  if (ECL_FIXNUMP(dims)) {
    ...
    FEwrong_type_nth_arg(ecl_make_fixnum(/*MAKE-ARRAY*/524), 1, dims,
                         cl_list(3, ECL_SYM("OR",616), ECL_SYM("LIST",483), ECL_SYM("FIXNUM",374)));
  }
  r = ecl_length(dims);
  ...
  for (i = 0, s = 1;  i < r;  i++, dims = ECL_CONS_CDR(dims)) {
    ...
    if (ecl_unlikely(!ECL_FIXNUMP(d) ||
                     ...
                     ecl_fixnum_greater(d, ecl_make_fixnum(ECL_ARRAY_DIMENSION_LIMIT))))
      {
        ...
        FEwrong_type_nth_arg(ecl_make_fixnum(/*MAKE-ARRAY*/524), 1, d, type);
      }
    j = ecl_fixnum(d);
    s *= (x->array.dims[i] = j);
    if (ecl_unlikely(s > ECL_ARRAY_TOTAL_LIMIT)) {
      ...
      FEwrong_type_key_arg(ecl_make_fixnum(/*MAKE-ARRAY*/524), ecl_make_fixnum(/*ARRAY-TOTAL-SIZE*/109),
               ...
               cl_object fillp, cl_object displ, cl_object disploff)
{
 ...
 AGAIN:
  ...
  if (ecl_unlikely(!ECL_FIXNUMP(dim) || ecl_fixnum_minusp(dim) ||
    ...
    cl_object type = ecl_make_integer_type(ecl_make_fixnum(0),
                                           ecl_make_fixnum(ECL_ARRAY_DIMENSION_LIMIT));
    FEwrong_type_nth_arg(ecl_make_fixnum(/*MAKE-ARRAY*/524), 1, dim, type);
  }
  d = ecl_fixnum(dim);
  if (aet == ecl_aet_bc) {
  ...
  if (Null(fillp)) {
    ...
    fillp = ecl_type_error(ECL_SYM("MAKE-ARRAY",524),"fill pointer",fillp,
                           cl_list(3,ECL_SYM("OR",616),cl_list(3,ECL_SYM("MEMBER",554),ECL_NIL,ECL_T),
                                   cl_list(3,ECL_SYM("INTEGER",439),ecl_make_fixnum(0),
                                           dim)));
    goto AGAIN;
  }
  x->vector.fillp = f;
...
{
 BEGIN:
  if (x == ECL_SYM("BASE-CHAR",122))
    return(ecl_aet_bc);
#ifdef ECL_UNICODE
  if (x == ECL_SYM("CHARACTER",224))
    return(ecl_aet_ch);
#endif
  else if (x == ECL_SYM("BIT",125))
    return(ecl_aet_bit);
  else if (x == ECL_SYM("EXT::CL-FIXNUM",1643))
    return(ecl_aet_fix);
  else if (x == ECL_SYM("EXT::CL-INDEX",1644))
    return(ecl_aet_index);
  else if (x == ECL_SYM("SINGLE-FLOAT",780) || x == ECL_SYM("SHORT-FLOAT",758))
    return(ecl_aet_sf);
  else if (x == ECL_SYM("DOUBLE-FLOAT",317))
    return(ecl_aet_df);
  else if (x == ECL_SYM("LONG-FLOAT",510))
    return(ecl_aet_lf);
#ifdef ECL_COMPLEX_FLOAT
  else if (x == ECL_SYM("SI::COMPLEX-SINGLE-FLOAT",1872))
    return(ecl_aet_csf);
  else if (x == ECL_SYM("SI::COMPLEX-DOUBLE-FLOAT",1873))
    return(ecl_aet_cdf);
  else if (x == ECL_SYM("SI::COMPLEX-LONG-FLOAT",1874))
    return(ecl_aet_clf);
#endif
  else if (x == ECL_SYM("EXT::BYTE8",1358))
    return(ecl_aet_b8);
  else if (x == ECL_SYM("EXT::INTEGER8",1363))
    return(ecl_aet_i8);
#ifdef ecl_uint16_t
  else if (x == ECL_SYM("EXT::BYTE16",1359))
    return(ecl_aet_b16);
  else if (x == ECL_SYM("EXT::INTEGER16",1364))
    return(ecl_aet_i16);
#endif
#ifdef ecl_uint32_t
  else if (x == ECL_SYM("EXT::BYTE32",1360))
    return(ecl_aet_b32);
  else if (x == ECL_SYM("EXT::INTEGER32",1365))
    return(ecl_aet_i32);
#endif
#ifdef ecl_uint64_t
  else if (x == ECL_SYM("EXT::BYTE64",1361))
    return(ecl_aet_b64);
  else if (x == ECL_SYM("EXT::INTEGER64",1366))
    return(ecl_aet_i64);
#endif
  else if (x == ECL_SYM("T",1))
...
ecl_displace(cl_object from, cl_object to, cl_object offset)
{
  ...
  if (ecl_unlikely(!ECL_FIXNUMP(offset) || ((j = ecl_fixnum(offset)) < 0))) {
    FEwrong_type_key_arg(ecl_make_fixnum(/*ADJUST-ARRAY*/85), ecl_make_fixnum(/*:DISPLACED-INDEX-OFFSET*/1242),
                         offset, ecl_make_fixnum(/*FIXNUM*/374));
  }
...
si_array_raw_data(cl_object x)
{
  ...
  if (to_array == ECL_NIL || ((to_array = ECL_CONS_CAR(to_array)) == ECL_NIL)) {
    ...
    output = si_make_vector(ECL_SYM("EXT::BYTE8",1358),
...
{
  if (ecl_unlikely(!ECL_ARRAYP(x)))
    FEwrong_type_argument(ecl_make_fixnum(/*ARRAY*/98), x);
...
ecl_array_rank(cl_object a)
{
  ...
  case t_bitvector:
    return 1;
  default:
    FEwrong_type_only_arg(ecl_make_fixnum(/*ARRAY-RANK*/106), a, ecl_make_fixnum(/*ARRAY*/98));
  }
}
...
ecl_array_dimension(cl_object a, cl_index index)
{
  ...
  case t_bitvector:
    ...
    return a->vector.dim;
  default:
    FEwrong_type_only_arg(ecl_make_fixnum(/*ARRAY-DIMENSION*/99), a, ecl_make_fixnum(/*ARRAY*/98));
  }
}
...
cl_array_total_size(cl_object a)
{
  if (ecl_unlikely(!ECL_ARRAYP(a)))
    FEwrong_type_only_arg(ecl_make_fixnum(/*ARRAY-TOTAL-SIZE*/109), a, ecl_make_fixnum(/*ARRAY*/98));
  {
#line 960
	const cl_env_ptr the_env = ecl_process_env();
#line 960
...
{
  if (ecl_unlikely(!ECL_ARRAYP(a)))
    FEwrong_type_only_arg(ecl_make_fixnum(/*ADJUSTABLE-ARRAY-P*/86), a, ecl_make_fixnum(/*ARRAY*/98));
  {
#line 968
	const cl_env_ptr the_env = ecl_process_env();
#line 968
...
cl_array_displacement(cl_object a)
{
  ...
  if (ecl_unlikely(!ECL_ARRAYP(a)))
    FEwrong_type_only_arg(ecl_make_fixnum(/*ARRAY-DISPLACEMENT*/102), a, ecl_make_fixnum(/*ARRAY*/98));
...
cl_svref(cl_object x, cl_object index)
{
  ...
  if (ecl_unlikely(ecl_t_of(x) != t_vector ||
    ...
    {
      FEwrong_type_nth_arg(ecl_make_fixnum(/*SVREF*/840),1,x,ecl_make_fixnum(/*SIMPLE-VECTOR*/776));
    }
  i = checked_index(ecl_make_fixnum(/*SVREF*/840), x, -1, index, x->vector.dim);
...
si_svset(cl_object x, cl_object index, cl_object v)
{
  ...
  if (ecl_unlikely(ecl_t_of(x) != t_vector ||
                   ...
                   (cl_elttype)x->vector.elttype != ecl_aet_object))
    {
      FEwrong_type_nth_arg(ecl_make_fixnum(/*SI::SVSET*/1174),1,x,ecl_make_fixnum(/*SIMPLE-VECTOR*/776));
    }
  i = checked_index(ecl_make_fixnum(/*SVREF*/840), x, -1, index, x->vector.dim);
...
mp_compare_and_swap_svref(cl_object x, cl_object index, cl_object old, cl_object new)
{
  ...
  if (ecl_unlikely(ecl_t_of(x) != t_vector ||
                   ...
                   (cl_elttype)x->vector.elttype != ecl_aet_object))
    {
      FEwrong_type_nth_arg(ecl_make_fixnum(/*MP::COMPARE-AND-SWAP-SVREF*/1512), 1, x, ecl_make_fixnum(/*SIMPLE-VECTOR*/776));
    }
  i = checked_index(ecl_make_fixnum(/*MP::COMPARE-AND-SWAP-SVREF*/1512), x, -1, index, x->vector.dim);
...
mp_atomic_incf_svref(cl_object x, cl_object index, cl_object increment)
{
  ...
  if (ecl_unlikely(ecl_t_of(x) != t_vector ||
                   ...
                   (cl_elttype)x->vector.elttype != ecl_aet_object))
    {
      FEwrong_type_nth_arg(ecl_make_fixnum(/*MP::ATOMIC-INCF-SVREF*/1518), 1, x, ecl_make_fixnum(/*SIMPLE-VECTOR*/776));
    }
  i = checked_index(ecl_make_fixnum(/*MP::ATOMIC-INCF-SVREF*/1518), x, -1, index, x->vector.dim);
...
cl_array_has_fill_pointer_p(cl_object a)
{
  ...
  case t_base_string:
    ...
    break;
  default:
    FEwrong_type_only_arg(ecl_make_fixnum(/*ARRAY-HAS-FILL-POINTER-P*/104),a,ecl_make_fixnum(/*ARRAY*/98));
  }
...
cl_fill_pointer(cl_object a)
{
  ...
  if (ecl_unlikely(!ECL_VECTORP(a)))
    FEwrong_type_only_arg(ecl_make_fixnum(/*FILL-POINTER*/364), a, ecl_make_fixnum(/*VECTOR*/900));
  if (ecl_unlikely(!ECL_ARRAY_HAS_FILL_POINTER_P(a))) {
    const char *type = "(AND VECTOR (SATISFIES ARRAY-HAS-FILL-POINTER-P))";
    FEwrong_type_only_arg(ecl_make_fixnum(/*FILL-POINTER*/364), a, ecl_read_from_cstring(type));
  }
...
si_fill_pointer_set(cl_object a, cl_object fp)
{
  ...
  if (ecl_unlikely(!ECL_VECTORP(a) || !ECL_ARRAY_HAS_FILL_POINTER_P(a))) {
    const char *type = "(AND VECTOR (SATISFIES ARRAY-HAS-FILL-POINTER-P))";
    FEwrong_type_nth_arg(ecl_make_fixnum(/*ADJUST-ARRAY*/85), 1, a,
  ...
  if (ecl_unlikely(!ECL_FIXNUMP(fp) || ((i = ecl_fixnum(fp)) < 0) ||
    ...
    FEwrong_type_key_arg(ecl_make_fixnum(/*ADJUST-ARRAY*/85), ecl_make_fixnum(/*:FILL-POINTER*/1261), fp, type);
  }
  a->vector.fillp = i;
