#line 1 "C:/work/c/memscan-dll/ecl-21.2.1-tests/ecl-rebuild/src/c/format.d"
...
{
  cl_error(7, ECL_SYM("SI::FORMAT-ERROR",1078),
           ECL_SYM(":FORMAT-CONTROL",1263), ecl_make_constant_base_string(s,-1),
           ECL_SYM(":CONTROL-STRING",1233), fmt->ctl_str,
           ECL_SYM(":OFFSET",1306), ecl_make_fixnum(fmt->ctl_index));
}
            ...
            int radix, int mincol, ecl_character padchar, ecl_character commachar)
{
  ...
  if (!ECL_FIXNUMP(x) && ecl_t_of(x) != t_bignum) {
    fmt_prepare_aux_stream(fmt);
    ecl_bds_bind(env, ECL_SYM("*PRINT-ESCAPE*",50), ECL_NIL);
    ecl_bds_bind(env, ECL_SYM("*PRINT-BASE*",47), ecl_make_fixnum(radix));
  ...
  fmt_prepare_aux_stream(fmt);
  ecl_bds_bind(env, ECL_SYM("*PRINT-RADIX*",58), ECL_NIL);
  ecl_bds_bind(env, ECL_SYM("*PRINT-BASE*",47), ecl_make_fixnum(radix));
  si_write_object(x, fmt->aux_stream);
  ecl_bds_unwind_n(env, 2);
...
fmt_radix(format_stack fmt, bool colon, bool atsign)
{
  ...
  if (fmt->nparam == 0) {
    ...
    fmt_prepare_aux_stream(fmt);
    ecl_bds_bind(env, ECL_SYM("*PRINT-RADIX*",58), ECL_NIL);
    ecl_bds_bind(env, ECL_SYM("*PRINT-BASE*",47), ecl_make_fixnum(10));
    si_write_object(x, fmt->aux_stream);
    ecl_bds_unwind_n(env, 2);
    s = 0;
...
fmt_character(format_stack fmt, bool colon, bool atsign)
{
  ...
  ensure_param(fmt, 0);
  x = fmt_advance(fmt);
  x = ecl_check_cl_type(ECL_SYM("FORMAT",389),x,t_character);
  if (!colon && !atsign) {
...
fmt_exponential_float(format_stack fmt, bool colon, bool atsign)
{
  ...
  if (sign < 0)
    ...
    ecl_write_char('+', fmt->stream);
  writestr_stream(b, fmt->stream);
  y = ecl_symbol_value(ECL_SYM("*READ-DEFAULT-FLOAT-FORMAT*",64));
  if (exponentchar < 0) {
    if (y == ECL_SYM("LONG-FLOAT",510)) {
      t = t_longfloat;
    } else if (y == ECL_SYM("DOUBLE-FLOAT",317)) {
      t = t_doublefloat;
    } else if (y == ECL_SYM("SINGLE-FLOAT",780)) {
      t = t_singlefloat;
    } else {
      t = t_singlefloat;
    }
...
#line 1931
	if (ecl_unlikely(narg < 2)) FEwrong_num_arguments(ecl_make_fixnum(1079));
#line 1931
  {
#line 1931
	#line 1931
	cl_object __value0 = doformat(narg, strm, string, args, TRUE);
#line 1931
	the_env->nvalues = 1;
#line 1931
...
#ifdef ECL_UNICODE
    string = ecl_type_error(ECL_SYM("FORMAT",389), "argument", string, ECL_SYM("STRING",807));
#else
  string = ecl_type_error(ECL_SYM("FORMAT",389), "argument", string, ECL_SYM("BASE-STRING",123));
#endif
  fmt.stream = strm;
  fmt_set_arg_list(&fmt, output);
  fmt.jmp_buf = &fmt_jmp_buf0;
  if (ecl_symbol_value(ECL_SYM("SI::*INDENT-FORMATTED-OUTPUT*",1023)) != ECL_NIL)
...
#line 2189
	...
	ecl_va_start(args, string, narg, 2);
#line 2189
	if (ecl_unlikely(narg < 2)) FEwrong_num_arguments(ecl_make_fixnum(389));
#line 2189
  if (Null(strm)) {
#ifdef ECL_UNICODE
    ...
    strm = ecl_symbol_value(ECL_SYM("*STANDARD-OUTPUT*",69));
  }
  if (ecl_stringp(strm)) {
    output = strm;
    if (!ECL_ARRAY_HAS_FILL_POINTER_P(output)) {
      cl_error(7, ECL_SYM("SI::FORMAT-ERROR",1078),
               ECL_SYM(":FORMAT-CONTROL",1263),
               ecl_make_constant_base_string("Cannot output to a non adjustable string.",-1),
               ECL_SYM(":CONTROL-STRING",1233), string,
               ECL_SYM(":OFFSET",1306), ecl_make_fixnum(0));
    }
  ...
  if (!Null(cl_functionp(string))) {
    ...
    _ecl_funcall4(ECL_SYM("SI::FORMATTER-AUX",1079), strm, string,
