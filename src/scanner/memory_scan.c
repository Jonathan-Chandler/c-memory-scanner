...
int mem_scan_init(mem_scan_t **this_scan, const char *window_title)
{
  ...
  *this_scan = 0;
  ...
  // initialize process memory
  if ((retval = proc_mem_init(&temp_scan->proc_mem, temp_scan->proc_info)) < 0)
  {
    return retval;
  }
...
int mem_scan_destroy(mem_scan_t **this_scan)
{
  ...
  {
    debug_error("Scan contained null proc_info");
    return -EINVAL;
  }
  ...
  if ( (retval = proc_mem_destroy(&(*this_scan)->proc_mem)) != 0)
  {
    return retval;
  }
...
int mem_scan_search(mem_scan_t *this_scan, uint8_t value_size, uint8_t *value)
{
  ...
  if (this_scan == 0)
  {
    debug_error("Receive null mem_scan");
    return -EINVAL;
  }

  if (this_scan->proc_mem == 0)
  {
    debug_error("Receive null proc_mem");
    return -EINVAL;
  }

  if ((retval = proc_mem_search(this_scan->proc_mem, value_size, value)) < 0)
  {
    return retval;
  }
...
{
  proc_mem_print(this_scan->proc_mem);
  proc_mem_print_matches(this_scan->proc_mem);
...
//int mem_scan_search(proc_info_t *procInfo)
//{
...
//}
//
//int mem_scan_update(proc_info_t *current_scan)
//{
...
//}
//
//int mem_scan_search(proc_info_t *current_scan, uint8_t *value, int value_size)
//{
//  mblock_t dummy;
...
//}
//
//int mem_scan_search_new(proc_info_t *current_scan, uint8_t *value, int value_size)
//{
//  mblock_t dummy;
//  mblock_t *current_block;
//  mblock_t *prev_block;
...
//  return 0;
//}
//int mem_scan_dump_results(proc_info_t *current_scan)
//{
//  mblock_t *current_block;
