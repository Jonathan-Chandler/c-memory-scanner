...
int ADD_CALL mem_mgr_page_search(mem_mgr_t *pMgr, const SIZE_T nSearchDataLength, const char *pSearchData, LPCVOID nStartAddress, bool *pbFoundPage, LPCVOID *pFoundAddress)
{
  ...
  *pFoundAddress = 0;
  ...
  printf("%s::%d::%s::pSearchData = 0x%08X\n", __FILE__, __LINE__, __func__, (uint32_t)pSearchData);
  printf("%s::%d::%s::nStartAddress = 0x%08X\n", __FILE__, __LINE__, __func__, (uint32_t)nStartAddress);
  printf("%s::%d::%s::pbFoundPage = 0x%08X\n", __FILE__, __LINE__, __func__, (uint32_t)pbFoundPage);
  printf("%s::%d::%s::pFoundAddress = 0x%08X\n\n", __FILE__, __LINE__, __func__, (uint32_t)pFoundAddress);
